        ...
        public void AddChild(List<MenuConfigEntity> menu)
        {
            ...
            foreach (var item in menu)
            {
                ChildMenu child = new ChildMenu()
                {
                    ...
                    MenuIcon =string.IsNullOrWhiteSpace(item.MenuIcon) ? null : new DataItemDetailBLL().GetItemValue("imgUrl") +  item.MenuIcon,
        ...
        public void AddChild(List<MenuConfigEntity> appMenu, List<AppSettingAssociationEntity> list)
        {
            ...
            foreach (var item in appMenu)
            {
                ...
                ChildMenu child = new ChildMenu()
                {
                    ...
                    ModuleCode = item.ModuleCode,
                    DataJson = string.IsNullOrEmpty(item.JsonData) ? "" : item.JsonData.Replace("'", "\"")
                };
                children.Add(child);
            }
            if (children.Count > 0)
