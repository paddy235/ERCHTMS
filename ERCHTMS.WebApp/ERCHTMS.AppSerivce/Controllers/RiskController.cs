    ...
    public class RiskController : BaseApiController
    {
        ...
        public object riskList([FromBody]JObject json)
        {
            try
            {
                ...
                pag.p_kid = "id riskId";
                pag.p_fields = @"riskdesc riskDescribe,grade riskLevel,risktype,majorname as riskpoint,worktask,
                                    process,equipmentname,parts,description,DangerSource,
                                    jobname,toolordanger,dangersourcetype,hjsystem,hjequpment,name,to_char(isconventional) as isconventional,to_char(isspecialequ) as isspecialequ,workcontent,checkprojectname";
                ...
                if (!string.IsNullOrEmpty(areaId))
                {
                    sql += " and instr(','|| districtid,'," + areaId + "')>0";
                }
        ...
        /// <returns></returns>
        [HttpPost]
        public object addRiskEntity([FromBody]JObject json)
        {
            try
            {
                string res = json.Value<string>("json");
                ...
                //string hjsystem = dy.data.hjsystem;
                //string hjequpment = dy.data.hjequpment;
                decimal itema = dy.data.itema;
                decimal itemb = dy.data.itemb;
                decimal itemc = dy.data.itemc;
                decimal itemr = dy.data.itemr;
                RiskAssessEntity risk = new RiskAssessEntity();
                //risk.Id = method == "add" ? "" : riskId;
                //if (method != "add" && status != 2)
                //{
        ...
        [HttpPost]
        public object GetRiskVersions([FromBody]JObject json) {
            try
            {
                var IsGdxy = new DataItemDetailBLL().GetDataItemListByItemCode("'VManager'").ToList();
                if (IsGdxy.Count > 0)
                {
                    return new { Code = 0, Count = -1, Info = "获取数据成功",data=true };
                }
                else
                {
                    return new { Code = 0, Count = -1, Info = "获取数据成功", data = false };
                }
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// 12.5 获取作业活动清单、设备设施清单
        /// </summary>
        /// <param name="json"></param>
        /// <returns></returns>
        [HttpPost]
        public object GetWorkDeviceList([FromBody]JObject json)
        {
            try
            {
                string res = json.Value<string>("json");
                dynamic dy = JsonConvert.DeserializeObject<ExpandoObject>(res);
                string userId = dy.userid;
                string type = dy.data.type; //风险类别
                string keyWord = dy.data.key;//搜索关键字
                ...
                pagination.p_kid = "id";
                pagination.p_fields = "name,activitystep,to_char(isconventional) as isconventional,to_char(isspecialequ) as isspecialequ,areaname,areacode,areaid,others,to_char(type) as type";
                pagination.p_tablename = "bis_baselisting a ";
                pagination.conditionJson = "1=1";
                pagination.page = (int)page;
                pagination.rows = (int)rows;
                DataTable dt = baselistingbll.GetPageListJson(pagination, JsonConvert.SerializeObject(new { type = type, name = keyWord }));
