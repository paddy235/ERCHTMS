    ...
    public class ApplyWorkController : BaseApiController
    {
            ...
            try
            {
                ...
                pagination.p_fields = @"s.iscommit,
                                          ...
                                          s.applyreturntime startdate,s.applyno,
                                            ...
                                            s.applytime,s.flowrolename,s.flowrole,s.flowdept,s.flowdeptname,s.nodeid,'' as approveuserid,'' as approveusername,s.outengineerid,'' as approveuserids";
                ...
                else if (actiontype == "1")//我的
                {
                    ...
                    strWhere1 += string.Format(" where  s.createuserorgcode='{0}' and  s.iscommit ='1' and  s.isover =0    ", currUser.OrganizeCode);
                    DataTable dt = intromissionbll.GetDataTableBySql(sql + strWhere1);
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        //获取下一步审核人
                        string str = manypowercheckbll.GetApproveUserId(dt.Rows[i]["nodeid"].ToString(), dt.Rows[i]["id"].ToString(), "", "", excutdept, outengineerdept, "", "", "", supervisordept, dt.Rows[i]["outengineerid"].ToString());
                ...
                else if (actiontype == "2")//待审核(批)
                {
                    ...
                    {
                        //我要处理的
                        pagination.conditionJson = string.Format("  s.id in ('{0}')", string.Join("','", applyids));
                    }
                    else
                    {
                        pagination.conditionJson = string.Format("  1!=1 ");
                ...
                for (int i = 0; i < data.Rows.Count; i++)
                {
                    ...
                    var supervisordept = string.IsNullOrEmpty(engineerEntity.SupervisorId) ? "" : departmentbll.GetEntity(engineerEntity.SupervisorId).DepartmentId;
                    //获取下一步审核人
                    string str = manypowercheckbll.GetApproveUserId(data.Rows[i]["nodeid"].ToString(), data.Rows[i]["outengineerid"].ToString(), "", "", excutdept, outengineerdept1, "", "", "", supervisordept, data.Rows[i]["outengineerid"].ToString());
                ...
                else
                {
                    ...
                    return new
                    {
                        ...
                        data = new
                        {
                            ...
                            enddate=string.IsNullOrWhiteSpace(data.Rows[0]["enddate"].ToString())?"": DateTime.Parse(data.Rows[0]["enddate"].ToString()).ToString("yyyy-MM-dd"),
        ...
        public object GetInfo([FromBody]JObject json)
        {
            try
            {
                ...
                else
                {
                    ...
                    List<CheckFlowData> nodeList = new AptitudeinvestigateinfoBLL().GetAppFlowList(id, "7", user);
                    return new
                    {
                        Code = 0,
                        Count = 1,
        ...
        public object saveInfo()
        {
            try
            {
                ...
                string endTime = dy.data.endDate;//计划竣工日期
                ...
                {
                    entity.APPLYENDTIME = DateTime.Parse(endTime);
                }
                entity.ISCOMMIT = "1";
                ...
                //更改申请信息状态
                mpcEntity = peoplereviewbll.CheckAuditPower(curUser, out status, moduleName, entity.OUTENGINEERID, false, entity.NodeId);
