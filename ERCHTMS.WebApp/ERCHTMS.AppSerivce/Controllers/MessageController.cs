    ...
    public class MessageController : BaseApiController
    {
        ...
        public object GetMessCountByUserId([FromBody]JObject json)
        {
            try
            {
                ...
                OperatorProvider.AppUserId = userId;  //设置当前用户
                Operator user = OperatorProvider.Provider.Current();
                var num = messagebll.GetMessCountByUserId(user);
                return new
                {
                    code = 0,
                    count = 1,
        ...
        /// <summary>
        /// 定时向EHS部门用户发送短消息提醒上报EHS信息
        /// </summary>
        /// <param name="json"></param>
        /// <returns></returns>
        [HttpPost]
        [HttpGet]
        public object ScheduleEHSInfoMsg()
        {
            var info = "消息发送成功。";
            try
            {
                OperatorProvider.AppUserId = "System";  //设置当前用户
                var bll = new InfoSubmitBLL();
                bll.SendMessage();
                
            }
            catch(Exception ex)
            {
                info = "消息发送失败。错误：" + ex.Message;
            }
