    ...
    public class SyncDataController : BaseApiController
    {
        ...
        public string SaveUser(string keyValue)
        {
            ...
            try
            {
                ...
                string jobid = "";
                if (!string.IsNullOrWhiteSpace(Planer))
                {
                ...
                if (!string.IsNullOrWhiteSpace(user.DutyId))
                {
                    var role = new RoleBLL().GetList(2).Where(x => x.RoleId == user.DutyId).FirstOrDefault();
                    if (role != null)
                    {
                        user.RoleId = role.RoleIds;
                        user.RoleName = role.RoleNames;
                    }
                    else
                    {
                ...
                if (user.RoleName.Contains("负责人")) user.UserType = "班组负责人";
                else user.UserType = "普通员工";
                ...
                try
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(dy.HeadIcon))
                    {
                        byte[] byteData = byteData = Convert.FromBase64String(dy.HeadIcon);
                        DataItemDetailBLL dd = new DataItemDetailBLL();
                ...
                {
                    System.IO.File.AppendAllText(HttpContext.Current.Server.MapPath("~/logs/" + fileName), DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + " ,同步用户签名失败,异常信息：" + ex.Message + ",用户信息：" + json + "\r\n");
                }
        ...
        public string SaveRole()
        {
            ...
            try
            {
                ...
                roleBll.SaveForm(role.RoleId, role);
