...
namespace ERCHTMS.AppSerivce.Controllers
{
    public class FirefightingController : BaseApiController
    {
        ...
        private FillRecordBLL fillrecordbll = new FillRecordBLL();
        private DataItemDetailBLL dataitemdetailbll = new DataItemDetailBLL();
        private FileInfoBLL fileInfoBLL = new FileInfoBLL();
        private DataItemDetailBLL dataItemDetailBLL = new DataItemDetailBLL();
        private TerminalDetectionRecordBLL terminaldetectionrecordbll = new TerminalDetectionRecordBLL();
        private TerminalDetectionProjectBLL terminaldetectionprojectbll = new TerminalDetectionProjectBLL();
        /// <summary>
        /// 获取消防设施列表
        /// </summary>
        ...
        public object GetFirefightingList([FromBody]JObject json)
        {
            try
            {
                ...
                //部门
                string ExamineDeptCode = dy.data.ExamineDeptCode;
                if (!string.IsNullOrEmpty(DutyDeptCode))
                {
                    pagination.conditionJson += string.Format(" and ExamineDeptCode like '{0}%'", ExamineDeptCode);
                }
        ...
        /// <returns></returns>
        [HttpPost]
        public object GetFillRecordList([FromBody]JObject json)
        {
            try
            {
                string res = json.Value<string>("json");
                ...
                Pagination pagination = new Pagination();
                pagination.p_kid = "ID";
                pagination.p_fields = "EquipmentId,FillDate,FillPerson,FillPersonId,Remark,cast(OperateType as varchar(2)) OperateType";
                pagination.p_tablename = "HRS_FILLRECORD";
                pagination.conditionJson = string.Format(" CREATEUSERORGCODE ='{0}'", curUser.OrganizeCode);
                pagination.page = page;//页数
                pagination.rows = rows;//行数
                pagination.sidx = "createdate";//排序字段
                ...
                if (!string.IsNullOrEmpty(sTime) || !string.IsNullOrEmpty(eTime))
                {
                    ...
                    //eTime = (Convert.ToDateTime(eTime).AddDays(1)).ToString("yyyy-MM-dd");
                    pagination.conditionJson += string.Format(" and FillDate between to_date('{0}','yyyy-MM-dd') and  to_date('{1}','yyyy-MM-dd')", sTime, eTime);
                }
                DataTable dt = fillrecordbll.GetPageList(pagination, null);
        ...
        /// <returns></returns>
        [HttpPost]
        public object GetTerminalDetectionRecordList([FromBody]JObject json)
        {
            try
            {
                string res = json.Value<string>("json");
        ...
        public object SaveFillRecord()
        {
            try
            {
                ...
                string keyValue = dy.data.Id;
                string str = JsonConvert.SerializeObject(dy.data);
                FillRecordEntity entity = JsonConvert.DeserializeObject<FillRecordEntity>(str);

                //HttpFileCollection files = ctx.Request.Files;//上传的文件 

                fillrecordbll.SaveForm(keyValue, entity);
            }
            catch (Exception)
            {
                return new { code = -1, count = 0, info = "保存失败" };
        ...
        /// <returns></returns>
        [HttpPost]
        public object SaveTerminalDetectionRecord()
        {
            try
            {
                string res = ctx.Request["json"];
                ...
                string keyValue = dy.data.Id;
                string str = JsonConvert.SerializeObject(dy.data);
                TerminalDetectionRecordEntity entity = JsonConvert.DeserializeObject<TerminalDetectionRecordEntity>(str);
                string str1 = JsonConvert.SerializeObject(dy.data.TerminalDetectionProjectEntity);
                List<TerminalDetectionProjectEntity> projects = JsonConvert.DeserializeObject<List<TerminalDetectionProjectEntity>>(str1);
                ...
                if (projects.Count > 0)
                {
                    foreach (var item in projects)
                    {
                        ...
                        item.RecordId = keyValue;
                        terminaldetectionprojectbll.SaveForm(item.Id, item);//保存明细
                    }
                    entity.DetectionResult = result;
                    entity.FilesId = entity.Id;
                    terminaldetectionrecordbll.SaveForm(keyValue, entity);//保存主表
                }
                HttpFileCollection files = ctx.Request.Files;//上传的文件 
                                                             //上传设备图片
                UploadifyFile(entity.Id, "TerminalDetectionRecord", files);
        ...
        public object GetFireEntity([FromBody]JObject json)
        {
            try
            {
                ...
                firefightingEntityApp.FillRecordList = GetFillRecordGrid(data.Id);
                firefightingEntityApp.TerminalDetectionRecordList = GetTerminaDetectionRecordGrid(data.Id);
        ...
        /// <param name="EquipmentId">消防设施ID</param>
        /// <returns></returns>
        public object GetFillRecordGrid(string EquipmentId)
        {
            object data = null;

            if (!string.IsNullOrWhiteSpace(EquipmentId))
            {
                var list = fillrecordbll.GetList(string.Format(" and EquipmentId='{0}' and rownum<5", EquipmentId)).OrderByDescending(x => x.CreateDate);
                data = list;
            }
        ...
        /// <param name="EquipmentId">消防设施ID</param>
        /// <returns></returns>
        public object GetTerminaDetectionRecordGrid(string EquipmentId)
        {
            object data = null;

            if (!string.IsNullOrWhiteSpace(EquipmentId))
            {
                var list = terminaldetectionrecordbll.GetList(string.Format(" and EquipmentId='{0}' and rownum<5", EquipmentId)).OrderByDescending(x => x.CreateDate);
                data = list;
            }
        ...
        /// <returns></returns>
        [HttpPost]
        public object GetFillEntity([FromBody]JObject json)
        {
            try
            {
                string res = json.Value<string>("json");
                ...
                string id = dy.data.Id;//ID
                var data = fillrecordbll.GetEntity(id);
        ...
        /// <returns></returns>
        [HttpPost]
        public object GetTerminalDetectionEntity([FromBody]JObject json)
        {
            try
            {
                string res = json.Value<string>("json");
                ...
                string id = dy.data.Id;//ID
                TerminalDetectionRecordEntity entity = terminaldetectionrecordbll.GetEntity(id);
                dynamic obj = new ExpandoObject();
                obj.Id = entity.Id;
                obj.EquipmentId = entity.EquipmentId;
                obj.DetectionPerson = entity.DetectionPerson;
                ...
                obj.DetectionPersonId = entity.DetectionPersonId;
                obj.Remark = entity.Remark;
                obj.DetectionUnit = entity.DetectionUnit;
                obj.DetectionResult = entity.DetectionResult;
                obj.DetectionSystem = entity.DetectionSystem;
                ...
                //获取检测项目
                var list = terminaldetectionprojectbll.GetList(string.Format(" and RecordId='{0}'", id)).OrderByDescending(x => x.CreateDate);
                obj.TerminalDetectionProjectEntity = list;
        ...
        public object GetDataItemListJson([FromBody]JObject json)
        {
            try
            {
                ...
                {
                    data = data.Where(t => t.ItemCode == Remark).OrderBy(t => t.SortCode).ToList();
                }
                return new { Code = 0, Count = 1, Info = "获取数据成功", data = data };
            }
            catch (Exception ex)
    ...
    public class FirefightingEntityApp
    {
        ...
        public object FillRecordList { get; set; }
        public object TerminalDetectionRecordList { get; set; }
    }
}