    ...
    public class EquipmentController : BaseApiController
    {
        ...
        public object GetEquipmentNormalEntity([FromBody]JObject json)
        {
            try
            {
                ...
                obj.file = pList;
                obj.RiskAssess = GetRiskAssess(entity.RelWord,curUser.OrganizeCode);
        ...
        /// <returns></returns>
        private object GetRiskAssess(string relword,string orgcode)
        {
            object data = null;

            if (!string.IsNullOrWhiteSpace(relword))
            {
                var list = new RiskAssessBLL().GetListFor(string.Format(" and createuserorgcode ='{0}' and status>0 and deletemark=0 and equipmentname=N'{1}'",orgcode, relword)).OrderByDescending(x => x.CreateDate);
        ...
        public object GetEquipmentEntity([FromBody]JObject json)
        {
            try
            {
                ...
                obj.CheckDateCycle = entity.CheckDateCycle;//检验周期
                ...
                obj.EmploySite = entity.EmploySite;//使用地点
                string EquipmentKind = "";
                var itemlist1 = dataitemdetailbll.GetDataItemListByItemCode("'EquipmentKind'");
                if (entity.EquipmentKind != null)
                {
                    var EquipmentKindEntity = itemlist1.Where(a => a.ItemValue == entity.EquipmentKind).ToList();
                    if (EquipmentKindEntity.Count() > 0) {
                        EquipmentKind = EquipmentKindEntity[0].ItemName;
                    }
                }
                //string EquipmentKind = itemlist1.Where(a => a.ItemValue == entity.EquipmentKind).ToList()[0].ItemName;
                obj.EquipmentKind = EquipmentKind;//设备类别
                string EquipmentBreed = "";
                var itemlist2 = dataitemdetailbll.GetDataItemListByItemCode("'EquipmentBreed'");
                if (entity.EquipmentBreed != null)
                {
                    var EquipmentBreedEntity = itemlist2.Where(a => a.ItemValue == entity.EquipmentBreed).ToList();
                    if (EquipmentBreedEntity.Count() > 0)
                    {
                        EquipmentBreed = EquipmentBreedEntity[0].ItemName;
                    }
                }
                //string EquipmentBreed = itemlist2.Where(a => a.ItemValue == entity.EquipmentBreed).ToList()[0].ItemName;
                obj.EquipmentBreed = EquipmentBreed;//设备品种
                obj.EmployDeptId = entity.EmployDeptId;//使用单位ID

                string EmployDept = "";
                if (entity.EmployDeptId != null)
                {
                    var deptEntity = departmentbll.GetEntity(entity.EmployDeptId);
                    if (deptEntity != null)
                    {
                        EmployDept = deptEntity.FullName;
                    }
                }
                obj.EmployDept = EmployDept;//使用单位

                var ControlMajor = "";
                var itemlist3 = dataitemdetailbll.GetDataItemListByItemCode("'SpecialtyType'");
                if (entity.ControlMajor != null)
                {
                    string[] cm = entity.ControlMajor.Split(',');
                    foreach (var s in cm)
                    {
                        var ControlMajorEntity = itemlist3.Where(a => a.ItemValue == s).ToList();
                        if (ControlMajorEntity.Count() > 0)
                        {
                            ControlMajor += ControlMajorEntity[0].ItemName + ",";
                        }
                    }
                    ControlMajor = ControlMajor.TrimEnd(',');
                }
                obj.ControlMajor = ControlMajor;//管控专业
                obj.CertificateNumber = entity.CertificateNumber;//操作人员证书编号
                obj.EquipmentRegisterNo = entity.EquipmentRegisterNo;//设备注册代码
                obj.MaintainUnit = entity.MaintainUnit;//维保单位
                obj.Location = entity.Location;//设备所在地
                obj.ExamineUnit = entity.ExamineUnit;//检验单位
                obj.ReportExamineDate = entity.ReportExamineDate != null ? entity.ReportExamineDate.Value.ToString("yyyy-MM-dd") : "";//报检日期
                obj.ExamineAppearDate = entity.ExamineAppearDate != null ? entity.ExamineAppearDate.Value.ToString("yyyy-MM-dd") : "";//监察单上报日期
                obj.AcceptState = entity.AcceptState;//受理状态
                obj.ExamineVerdict = entity.ExamineVerdict;//检验结论
                obj.ReportNumber = entity.ReportNumber;//检验报告编号
                //使用状况
                string state = entity.State;
                switch (state)
                {
        ...
        public object GetExamineRecordList([FromBody]JObject json)
        {
            try
            {
                ...
                Pagination pagination = new Pagination();
                pagination.p_kid = "ID";
                pagination.p_fields = "CreateUserId,CreateUserDeptCode,CreateUserOrgCode,CreateDate,createusername,equipmentid,ExamineUnit,ExamineType,ExamineDate,ExaminePeriod,ExamineVerdict,ReportNumber";
                ...
                string searchWord = dy.data.searchWord;
                if (!string.IsNullOrEmpty(searchWord))
                {
                    pagination.conditionJson += string.Format(" and ReportNumber like'%{0}%'", searchWord);
                }
                ...
                if (!string.IsNullOrEmpty(startTime))
                {
                    ...
                    pagination.conditionJson += string.Format(" and ExamineDate between to_date('{0}','yyyy-MM-dd') and  to_date('{1}','yyyy-MM-dd')", startTime, endTime);
                }
                DataTable dt = EquipmentExaminebll.GetPageList(pagination);
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    item.CreateUserDeptCode = dr["createuserdeptcode"].ToString();
                    item.CreateUserOrgCode = dr["createuserorgcode"].ToString();
                    item.CreateDate = dr["createdate"].ToString();
                    item.CreateUserName = dr["createusername"].ToString();
                    item.EquipmentId = dr["equipmentid"].ToString();
                    item.ExamineUnit = dr["examineunit"].ToString();
                    item.ExamineType = dr["examinetype"].ToString();
                    item.ExamineDate = dr["examinedate"].ToString();
                    item.ExaminePeriod = dr["examineperiod"].ToString();
                    item.ExamineVerdict = dr["examineverdict"].ToString();
                    item.ReportNumber = dr["reportnumber"].ToString();
                    IList<Photo> pList = new List<Photo>(); //附件
        ...
        /// <returns></returns>
        [HttpPost]
        public object GetTechnologyList([FromBody]JObject json)
        {
            try
            {
                string res = json.Value<string>("json");
                ...
                {
                    pagination.conditionJson += string.Format(" and TechnologyName like'%{0}%'", Name);
                }
                DataTable dt = EquipmentTechnologybll.GetPageList(pagination);
                List<object> data = new List<object>();
                foreach (DataRow dr in dt.Rows)
                {
                    dynamic item = new ExpandoObject();
                    ...
                    item.CreateUserName = dr["createusername"].ToString();
                    item.EquipmentId = dr["equipmentid"].ToString();
                    item.TechnologyName = dr["technologyname"].ToString();
                    item.Remaek = dr["remaek"].ToString();
                    IList<Photo> pList = new List<Photo>(); //附件
                    if (!string.IsNullOrEmpty(dr["id"].ToString()))
                    {
                        DataTable file = fileInfoBLL.GetFiles(dr["id"].ToString());
        ...
        /// <returns></returns>
        [HttpPost]
        public object GetJurisdiction([FromBody]JObject json)
        {
            try
            {
                string res = json.Value<string>("json");
                ...
                var IsQx = "0";//是否有权限
                DataItemModel ehsDepart = dataitemdetailbll.GetDataItemListByItemCode("'EquipmentJurisdiction'").Where(p => p.ItemName == curUser.OrganizeId).ToList().FirstOrDefault();
                if (ehsDepart != null)
                {
                    if (ehsDepart.ItemValue.Contains(curUser.DeptId))
                    {
                        if (!string.IsNullOrWhiteSpace(ehsDepart.ItemCode))
                        {

                            var roleArr = curUser.RoleName.Split(','); //当前人员角色
                            var roleName = ehsDepart.ItemCode; //审核角色
                ...
                if (!string.IsNullOrEmpty(EquipmentId))
                {
                    ...
                    if (entity.OperUserID == curUser.UserId|| entity.SecurityManagerUserID == curUser.UserId)
                    {
                        IsQx = "1";
                    }
                }
                return new { code = 0, info = "获取数据成功", data = IsQx };
            }
            catch (Exception ex)
            {
                return new { code = -1, info = ex.Message, count = 0 };
        ...
        public object GetEquipmentOperUser([FromBody]JObject json)
        {
            try
            {
                ...
                pagination.p_tablename = @"v_userinfo a left join (select id,filepath,t.userid,certname,Gender,certnum,senddate,sendorgan,years,realname,identifyid,deptname,enddate from BIS_CERTIFICATE t  left join v_userinfo u on t.userid=u.userid where t.certtype='特种设备作业人员证') b 
...
where d.checkobjecttype like '%0%' and d.checkobjectid like '%{0}%' group by t.id) ", EquipmentId);
        ...
        /// <returns></returns>
        [HttpPost]
        public object GetRemoteEquipmentList([FromBody]JObject json)
        {
            try
            {
                ...
                string equid = res.Contains("equid") ? dy.data.equid : "";
                ...
                if (!string.IsNullOrEmpty(equid))
                {
                    strWhere += string.Format(@" and  parentid = '{0}' ", equid);
                }
                string sql = string.Format("select  id, facname  equname ,parentid , id  equid ,faccode equcode from factbfacility t where {0} order by faccode", strWhere);
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    {
                        var childDt = dbBase.FindTable(string.Format("select  id from factbfacility t where parentid ='{0}' ", item["id"].ToString()));
                        item["isparent"] = childDt.Rows.Count > 0;
                        DataRow crow = clonedt.NewRow();
                        crow["equcode"] = item["equcode"];
                        crow["equname"] = item["equname"];
                        crow["parentid"] = item["parentid"];
                        crow["equid"] = item["equid"];
                        crow["isparent"] = item["isparent"];
                        clonedt.Rows.Add(crow);
                    }
                }

                return new { code = 0, info = "获取数据成功", count = clonedt.Rows.Count, data = clonedt };
            }
            catch (Exception ex)
            {
                return new { code = -1, info = ex.Message, count = 0 };
