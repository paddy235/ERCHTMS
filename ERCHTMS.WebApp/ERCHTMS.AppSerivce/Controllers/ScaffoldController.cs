...
namespace ERCHTMS.AppSerivce.Controllers
{
    public class ScaffoldController : BaseApiController
    {
        ...
        public object GetList([FromBody]JObject json)
        {
            try
            {
                ...
                if (!user.IsSystem)
                {
                    ...
                    if (curUser.RoleName.Contains("公司") || curUser.RoleName.Contains("厂级") || !string.IsNullOrEmpty(isAllDataRange))
                    {
                ...
                //        }
                //        pagination.conditionJson += string.Format("  and a.id in(select s.id from bis_scaffold s where s.flowremark='1' and s.specialtytype in({2}) and s.AuditState in(1,4,6) and s.flowdeptid like '%{0}%' and ({1}) union all select s.id from bis_scaffold s where (s.flowremark!='1' or s.flowremark is null) and s.AuditState in(1,4,6) and s.flowdeptid like '%{0}%' and ({1}))", user.DeptId, roleWhere, specialtytype);
                //    }
                //    else
                //    {
                //        //当前有审核权限的部门及角色，才可查看
        ...
        /// <returns></returns>
        [HttpPost]
        public object GetScaffodTypeName()
        {
            try
            {
                IList<DataItemModel> dataitemlist = dataitemdetailbll.GetDataItemListByItemCode("ScaffoldType").ToList();
                IList<Dataitem> data = new List<Dataitem>();
                foreach (var item in dataitemlist)
                {
                    Dataitem dataitem = new Dataitem();
                    dataitem.itemname = item.ItemName;
                    dataitem.itemvalue = item.ItemValue;
                    dataitem.min = Convert.ToDouble(item.ItemCode.Split('-')[0]);
                    dataitem.max = Convert.ToDouble(item.ItemCode.Split('-')[1]);
                    data.Add(dataitem);
                }
        ...
        public object GetLedgerList([FromBody]JObject json)
        {
            try
            {
                ...
                var historyCount = scaffoldbll.GetLedgerList(pagination, JsonConvert.SerializeObject(dy.data), "app").Rows.Count;
                var query = new
                {
                    SetupCompanyId = dy.data.SetupCompanyId,
                    SetupCompanyCode = dy.data.SetupCompanyCode,
                    DismentleCompanyId = dy.data.DismentleCompanyId,
                    DismentleCompanyCode = dy.data.DismentleCompanyCode,
                    ActSetupStartDate = dy.data.ActSetupStartDate,
                    ActSetupEndDate = dy.data.ActSetupEndDate,
                    DismentleStartDate = dy.data.DismentleStartDate,
                    DismentleEndDate = dy.data.DismentleEndDate,
                    CheckStartDate = dy.data.CheckStartDate,
                    CheckEndDate = dy.data.CheckEndDate,
                    LedgerType = dy.data.LedgerType,     //0-在用 1-已拆除
                    OutProjectName = dy.data.OutProjectName,
                    applynumber = dy.data.applynumber,
                    Working = "1"
                };
                var workingCount = scaffoldbll.GetLedgerList(pagination, JsonConvert.SerializeObject(query), "app").Rows.Count;
                Dictionary<string, string> dict_props = new Dictionary<string, string>();
                JsonSerializerSettings settings = new JsonSerializerSettings
                {
                    ContractResolver = new LowercaseContractResolver(dict_props), //转小写，并对指定的列进行自定义名进行更换
        ...
        public object GetForm([FromBody]JObject json)
        {
            try
            {
                ...
                if (scaffoleEntity.SpecialtyType != null)
                    scaffoleEntity.SpecialtyTypeName = scaffoldbll.getName(scaffoleEntity.SpecialtyType, "SpecialtyType");
                string webUrl = new DataItemDetailBLL().GetItemValue("imgUrl");
                ...
                try
                {
                    model.min = Convert.ToDouble(dataitemdetailbll.GetItemCode(model.SetupTypeName).Split('-')[0]);
                    model.max = Convert.ToDouble(dataitemdetailbll.GetItemCode(model.SetupTypeName).Split('-')[1]);
                }
                catch (Exception)
                {
                    model.min = 0;
                ...
                if (scaffoleEntity.ScaffoldType == 0)
                {
                    ...
                    {
                        modulename = "(搭设申请-内部-" + scaffoleEntity.SetupTypeName + ")审核";
                    }
                    else
                    {
                        modulename = "(搭设申请-外包-" + scaffoleEntity.SetupTypeName + ")审核";
                    }
                }
                if (scaffoleEntity.ScaffoldType == 1)
                {
                    if (scaffoleEntity.SetupCompanyType == 0)
                    {
                        modulename = "(搭设验收-内部-" + scaffoleEntity.SetupTypeName + ")审核";
                    }
                    else
                    {
                        modulename = "(搭设验收-外包-" + scaffoleEntity.SetupTypeName + ")审核";
                    }
                }
                if (scaffoleEntity.ScaffoldType == 2)
                {
                    if (scaffoleEntity.SetupCompanyType == 0)
                    {
                        modulename = "(搭设拆除-内部-" + scaffoleEntity.SetupTypeName + ")审核";
                    }
                    else
                    {
                        modulename = "(搭设拆除-外包-" + scaffoleEntity.SetupTypeName + ")审核";
                    }
                }
                var nodelist = scaffoldbll.GetAppFlowList(scaffoleEntity.Id, modulename);
                model.CheckFlow = nodelist;
                #endregion

                #region 获取执行情况

                IList<FireWaterCondition> conditionlist = firewaterbll.GetConditionList(model.Id).OrderBy(t => t.CreateDate).ToList();
        ...
        public object LedgerOp()
        {
            try
            {
                ...
                HttpFileCollection files = HttpContext.Current.Request.Files;
                string conditionid = Guid.NewGuid().ToString();
                UploadifyFile(conditionid, "", files);
                scaffoldbll.LedgerOp(dy.data.keyvalue, dy.data.ledgertype, dy.data.type, dy.data.worktime, dy.data.issendmessage, dy.data.conditioncontent, conditionid, dy.data.iscomplete);
                return new { code = 0, data = "", info = "操作成功" };
            }
            catch (Exception ex)
            {
