...
//登录
.controller('LoginCtrl', ['$scope', '$state', '$timeout', 'md5', '$frameworkTopAlert', '$frameworkHttp', 'ApiUrl', 'UserInfo', 'lrmIM',
    function ($scope, $state, $timeout, md5, $frameworkTopAlert, $frameworkHttp, ApiUrl, UserInfo, lrmIM) {
        $scope.data = {};
        $scope.logining = false;
        $scope.doLogin = function () {
            if ($scope.data.username == undefined || $scope.data.username == "") {
                $frameworkTopAlert.show({ text: "请输入账号" });
                return false;
            }
            if ($scope.data.password == undefined || $scope.data.password == "") {
                $frameworkTopAlert.show({ text: "请输入密码" });
                return false;
            }
            $scope.logining = true;
            $frameworkHttp.post({
                ...
                "success": function (data) {
                    ...
                    if (data.status.code != 0) {
                        $frameworkTopAlert.show({ text: data.status.desc });
                    }
                    else {
                ...
                "error": function () {
                    $scope.logining = false;
                    $frameworkTopAlert.show({ text: "通信失败" });
                }
            });
        };
    }])
//主页
.controller('HomeCtrl', [
    '$scope', '$frameworkPageModal', 'HomeApps', 'Announces',
  function ($scope, $frameworkPageModal, HomeApps, Announces) {
      $scope.homeApps = HomeApps.all();
      $scope.announces = Announces.all();
      //打开应用列表
      $scope.closePageModel = $frameworkPageModal($scope, 'templates/homeApps/');
  }])
//商机管理
.controller('BusinesssCtrl', [
    '$scope', '$timeout', '$ionicModal', '$frameworkPopup', '$ionicLoading', '$frameworkTopAlert', '$frameworkTriggerRefresh', '$frameworkSelectModal', '$frameworkDataIsAll', 'lrmBusinesss', 'lrmBaseInfo',
  function ($scope, $timeout, $ionicModal, $frameworkPopup, $ionicLoading, $frameworkTopAlert, $frameworkTriggerRefresh, $frameworkSelectModal, $frameworkDataIsAll, lrmBusinesss, lrmBaseInfo) {
      $scope.data = lrmBusinesss.getList();
      //刷新数据
      $timeout(function () {
          $frameworkTriggerRefresh.triggerRefresh('businesslist-content');
      }, 450);
      ...
      $scope.closeEditModal = function () {
          if ($frameworkDataIsAll.isHave($scope.editData)) {
              $frameworkPopup.confirm({
      ...
      $scope.doSubmit = function () {
          var res = $frameworkDataIsAll.isAll($scope.editData, lrmBusinesss.getEditDataEx());
          if (res != null) {
              $frameworkTopAlert.show({ text: res.name + " 不能为空" });
          } else {
              $ionicLoading.show();
...
.controller('CustomersCtrl', [
  '$scope', '$timeout', '$ionicModal', '$ionicLoading', '$frameworkTopAlert', '$frameworkTriggerRefresh', '$frameworkPopup', '$frameworkSelectModal', '$frameworkDataIsAll', 'lrmBaseInfo', 'lrmCustomers'
  , function ($scope, $timeout, $ionicModal, $ionicLoading, $frameworkTopAlert, $frameworkTriggerRefresh, $frameworkPopup, $frameworkSelectModal, $frameworkDataIsAll, lrmBaseInfo, lrmCustomers) {
      $scope.data = lrmCustomers.getList();
      //console.log(JSON.stringify(lrmCustomers.getList()));
      //刷新数据
      $timeout(function () {
          $frameworkTriggerRefresh.triggerRefresh('customerlist-content');
      }, 450);
      $scope.doRefresh = function () {
      ...
      $scope.submitCust = function () {
          var res = $frameworkDataIsAll.isAll($scope.editData, lrmCustomers.getEditDataEx());
          if (res != null) {
              $frameworkTopAlert.show({ text: res.name + "不能为空" });
          } else {
              $ionicLoading.show();
...
.controller('WorkflowCtrl', [
    '$scope', '$frameworkPageModal', 'lrmWFlows',
  function ($scope, $frameworkPageModal, lrmWFlows) {
      $scope.wflows = lrmWFlows.all();
      $scope.closeFlowPageModel = $frameworkPageModal($scope, 'templates/homeApps/workflow/');
  }])
//发起流程
.controller('flowDesignCtrl', [
    '$scope', '$timeout', '$frameworkTriggerRefresh', '$ionicModal', '$frameworkDataIsAll', '$frameworkTopAlert', '$ionicLoading', 'lrmFlowDesigns',
  function ($scope, $timeout, $frameworkTriggerRefresh, $ionicModal, $frameworkDataIsAll, $frameworkTopAlert, $ionicLoading, lrmFlowDesigns) {
      $scope.data = lrmFlowDesigns.getList();
      //刷新数据
      $timeout(function () {
          $frameworkTriggerRefresh.triggerRefresh('flowDesignlist-content');
      }, 200);
      $scope.doRefresh = function () {
...
.controller('flowRoughdraftCtrl', [
    '$scope', '$timeout', '$frameworkHttp', '$frameworkTopAlert', '$frameworkTriggerRefresh', '$frameworkPopup', 'ApiUrl', 'lrFlowRoughdrafts',
  function ($scope, $timeout, $frameworkHttp, $frameworkTopAlert, $frameworkTriggerRefresh, $frameworkPopup, ApiUrl, lrFlowRoughdrafts) {
      $scope.dat = lrFlowRoughdrafts.getList();
      //刷新数据
      $timeout(function () {
          $frameworkTriggerRefresh.triggerRefresh('flowRoughdraftlist-content');
      }, 200);
      $scope.doRefresh = function () {
...
.controller('flowProcessCtrl', [
    '$scope', '$timeout', '$frameworkHttp', '$frameworkTopAlert', '$frameworkTriggerRefresh', '$ionicModal', 'ApiUrl', 'lrWorkflows',
  function ($scope, $timeout, $frameworkHttp, $frameworkTopAlert, $frameworkTriggerRefresh, $ionicModal, ApiUrl, lrWorkflows) {
      $scope.dat = lrWorkflows.getList();
      //刷新数据
      $timeout(function () {
          $frameworkTriggerRefresh.triggerRefresh('flowProcesslist-content');
      }, 200);
      $scope.doRefresh = function () {
...
.controller('flowBefProcessCtrl', [
    '$scope', '$timeout', '$frameworkHttp', '$frameworkTopAlert', '$frameworkTriggerRefresh', '$ionicModal', 'UserInfo', 'ApiUrl', 'lrFlowBefProcesss',
  function ($scope, $timeout, $frameworkHttp, $frameworkTopAlert, $frameworkTriggerRefresh, $ionicModal, UserInfo, ApiUrl, lrFlowBefProcesss) {
      ...
      //刷新数据
      $timeout(function () {
          $frameworkTriggerRefresh.triggerRefresh('flowBefProcesslist-content');
      }, 200);
      $scope.doRefresh = function () {
...
.controller('flowAftProcessCtrl', [
    '$scope', '$timeout', '$frameworkHttp', '$frameworkTopAlert', '$frameworkTriggerRefresh', '$ionicModal', 'ApiUrl', 'lrFlowAftProcesss',
  function ($scope, $timeout, $frameworkHttp, $frameworkTopAlert, $frameworkTriggerRefresh, $ionicModal, ApiUrl, lrFlowAftProcesss) {
      $scope.dat = lrFlowAftProcesss.getList();
      //刷新数据
      $timeout(function () {
          $frameworkTriggerRefresh.triggerRefresh('flowAftProcesslist-content');
      }, 200);
      $scope.doRefresh = function () {
...
.controller('flowDelegateCtrl', [
  '$scope', '$timeout', '$frameworkTriggerRefresh', 'ApiUrl', 'lrFlowDelegates',
  function ($scope, $timeout, $frameworkTriggerRefresh, ApiUrl, lrFlowDelegates) {
      $scope.dat = lrFlowDelegates.getList();
      //刷新数据
      $timeout(function () {
          $frameworkTriggerRefresh.triggerRefresh('flowDelegatelist-content');
      }, 200);
      $scope.doRefresh = function () {
...
//即时通讯
  .controller('IMCtrl', ['$scope', '$ionicScrollDelegate', '$frameworkPageModal', 'lrmIM', '$frameworkFormatDate', function ($scope, $ionicScrollDelegate, $frameworkPageModal, lrmIM, $frameworkFormatDate) {
      ...
      $scope.formatDate = function (date) {
          return $frameworkFormatDate(date, "MM-dd hh:mm");
      };
      $scope.closeMessageDetailModel = $frameworkPageModal($scope, 'templates/homeApps/wechat/wechatDetails.html', function (otherUserId) {//打开聊天窗口
...
.controller('CasesCtrl', [
    '$scope', '$frameworkPageModal', 'Cases', function ($scope, $frameworkPageModal, Cases) {
        $scope.cases = Cases.all();
        //打开应用列表
        $scope.closePageModel = $frameworkPageModal($scope, 'templates/cases/', function (id) { $scope.caseItem = Cases.get(id); });
    }])
...
.controller('NoticeCtrl', ['$scope', '$ionicModal', function ($scope, $ionicModal) {
    $scope.allNotices = {
        "btnNoRead": [
          { title: '测试数据', content: '测试数据', datatime: '63月12日', id: '13', type: '1' }],
        "btnRead": [
          { title: '测试数据', content: '测试数据', datatime: '73月27日', id: '14', type: '2' }]
    };
...
.controller('NoticeListCtrl', function ($scope) {
  ...
  //订单
  .controller('lrOrderCtrl', ['$scope', '$timeout', '$frameworkTriggerRefresh', '$ionicModal', 'lrmOrderInfos',
    function ($scope, $timeout, $frameworkTriggerRefresh, $ionicModal, lrmOrderInfos) {
        //console.log(JSON.stringify(lrmOrderInfos.getList()));
        $scope.data = lrmOrderInfos.getList();
        //刷新数据
        $timeout(function () {
            $frameworkTriggerRefresh.triggerRefresh('orderlist-content');
        }, 450);
        $scope.doRefresh = function () {
  ...
  //收款
  .controller('lrReceivableCtrl', ['$scope', '$timeout', '$frameworkTriggerRefresh', '$ionicModal', 'lrmReceivables',
    function ($scope, $timeout, $frameworkTriggerRefresh, $ionicModal, lrmReceivables) {
        $scope.data = lrmReceivables.getList();
        //刷新数据
        $timeout(function () {
            $frameworkTriggerRefresh.triggerRefresh('receivablelist-content');
        }, 450);
        $scope.doRefresh = function () {
  ...
  //收款报表
  .controller('lrReceivableReportCtrl', ['$scope', '$timeout', '$frameworkTriggerRefresh', 'lrmReceivableReports',
    function ($scope, $timeout, $frameworkTriggerRefresh, lrmReceivableReports) {
        //console.log(lrmReceivableReports.getList());
        $scope.data = lrmReceivableReports.getList();
        //console.log($scope.data);
        //刷新数据
        $timeout(function () {
            $frameworkTriggerRefresh.triggerRefresh('receivableReportlist-content');
        }, 450);
        $scope.doRefresh = function () {
  ...
  .controller('PersonCenterCtrl', [
    '$scope', '$state', '$frameworkPageModal', '$frameworkPopup', '$frameworkHttp', 'UserInfo', 'ApiUrl',
    function ($scope, $state, $frameworkPageModal, $frameworkPopup, $frameworkHttp, UserInfo, ApiUrl) {
        $scope.userinfo = UserInfo.get();
        ...
        //打开个人信息列表
        $scope.closePageModel = $frameworkPageModal($scope, 'templates/personCenter/');
    }]);
