...
//客户列表信息
  .factory('lrmCustomers', ['$frameworkFormatDate', 'AreaInfo', '$frameworkGetDataItem', '$frameworkHttp', 'ApiUrl', '$frameworkTopAlert',
    function ($frameworkFormatDate, AreaInfo, $frameworkGetDataItem, $frameworkHttp, ApiUrl, $frameworkTopAlert) {
    //客户列表
    var custList = {
      records: 0,
      page: 1,
    ...
    function translateData(data, obj) {
      //console.log(data);
        for (var i in data) {
          ...
          data[i].f_LastDate = item.f_ModifyDate == null ? $frameworkFormatDate(item.f_CreateDate, 'MM-dd') : $frameworkFormatDate(item.f_ModifyDate, 'MM-dd');
          data[i].f_CreateDate = $frameworkFormatDate(item.f_CreateDate, 'yyyy-MM-dd hh:mm');
          data[i].f_ModifyDate = $frameworkFormatDate(item.f_ModifyDate, 'yyyy-MM-dd hh:mm');
          obj.push(data[i]);
        }
    };
    //获取数据
    function getData(page,queryData, obj,callback) {
        ...
        "error": function () {
            $frameworkTopAlert.show({ text: "刷新失败" });
        },
        "finally": function () {
          callback();
        }
    ...
    return {
      ...
      remove: function (customer) {
          ...
          "error": function () {
              $frameworkTopAlert.show({ text: "删除失败！" });
          }
        });
      },
      editSubmit: function (editData,callback) {
          ...
          "isverify": true,
          "success": function (data) {
              $frameworkTopAlert.show({ text: "保存成功！" });
          },
          "error": function () {
              $frameworkTopAlert.show({ text: "保存失败！" });
          },
          "finally":function(){
            callback();
          }
...
//商机列表信息
  .factory('lrmBusinesss', ['$frameworkFormatDate', '$frameworkGetDataItem', '$frameworkHttp', 'ApiUrl', '$frameworkTopAlert',
    function ($frameworkFormatDate, $frameworkGetDataItem, $frameworkHttp, ApiUrl, $frameworkTopAlert) {
    //商机列表数据
    var businessList ={
      records: 0,
      page: 1,
    ...
    function translateData(data,obj) {
      //console.log(data);
        for (var i in data) {
          var item = data[i];
          data[i].StageName =  chancePhases[data[i].StageId].f_ItemName;
          data[i].BgStageColor = chancePhases[data[i].StageId].bgColor;
          data[i].SourceName = chanceSource[data[i].SourceId].f_ItemName;
          data[i].LastDate = $frameworkFormatDate(item.ModifyDate, 'MM-dd');
          data[i].CreateDate = $frameworkFormatDate(item.CreateDate, 'yyyy-MM-dd hh:mm');
          data[i].ModifyDate = $frameworkFormatDate(item.ModifyDate, 'yyyy-MM-dd hh:mm');
          data[i].DealDate = $frameworkFormatDate(item.DealDate, 'yyyy-MM-dd');
          obj.push(data[i]);
        }
    };
    //获取数据方法
...
//发起流程
.factory('lrmFlowDesigns', ['$frameworkFormatDate', '$frameworkHttp', 'ApiUrl', '$frameworkTopAlert', '$frameworkGetDataItem',
  function ($frameworkFormatDate, $frameworkHttp, ApiUrl, $frameworkTopAlert, $frameworkGetDataItem) {
  //发起流程列表数据
  var flowdesignList = {
    records: 0,
    page: 1,
  ...
  function translateData(data, obj) {
    for (var i in data) {
      var item = data[i];
      data[i].f_createdate = $frameworkFormatDate(item.f_createdate, 'MM-dd');
      obj.push(data[i]);
    }
  };
  //获取数据方法
...
//草稿流程
.factory('lrFlowRoughdrafts', ['$frameworkFormatDate', '$frameworkHttp', 'ApiUrl', '$frameworkTopAlert', '$frameworkGetDataItem',
  function ($frameworkFormatDate, $frameworkHttp, ApiUrl, $frameworkTopAlert, $frameworkGetDataItem) {
    //草稿流程列表数据
...
//我的流程列表
.factory('lrWorkflows', ['$frameworkFormatDate', '$frameworkHttp', 'ApiUrl', '$frameworkTopAlert', '$frameworkGetDataItem',
  function ($frameworkFormatDate, $frameworkHttp, ApiUrl, $frameworkTopAlert, $frameworkGetDataItem) {
    //我的流程列表数据
    var workflowList = {
        records: 0,
...
//代办流程
.factory('lrFlowBefProcesss', ['$frameworkFormatDate', '$frameworkHttp', 'ApiUrl', '$frameworkTopAlert', '$frameworkGetDataItem',
  function ($frameworkFormatDate, $frameworkHttp, ApiUrl, $frameworkTopAlert, $frameworkGetDataItem) {
    //代办流程列表数据
...
//已办流程
.factory('lrFlowAftProcesss', ['$frameworkFormatDate', '$frameworkHttp', 'ApiUrl', '$frameworkTopAlert', '$frameworkGetDataItem',
  function ($frameworkFormatDate, $frameworkHttp, ApiUrl, $frameworkTopAlert, $frameworkGetDataItem) {
    //已办流程列表数据
...
//工作委托
.factory('lrFlowDelegates', ['$frameworkFormatDate', '$frameworkHttp', 'ApiUrl', '$frameworkTopAlert', '$frameworkGetDataItem',
  function ($frameworkFormatDate, $frameworkHttp, ApiUrl, $frameworkTopAlert, $frameworkGetDataItem) {
  //发起流程列表数据
  ...
  function translateData(data, obj) {
      for (var i in data) {
          var item = data[i];
          data[i].f_createdate = $frameworkFormatDate(item.f_createdate, 'MM-dd');
          data[i].f_begindate = $frameworkFormatDate(item.f_begindate, 'MM-dd');
          data[i].f_enddate = $frameworkFormatDate(item.f_enddate, 'MM-dd');
          obj.push(data[i]);
      }
  };
  //获取数据方法
...
//即时通讯
    .factory('lrmIM', ['$q', 'UserInfo', 'SignalrUrl', '$frameworkLoadJs', '$frameworkFormatDate', function ($q, UserInfo, SignalrUrl, $frameworkLoadJs, $frameworkFormatDate) {
        var imServer = {
        ...
        function LastUserListToArray()
        {
            ...
            for (var i in imServer.lastUserList) {
                imServer.lastUserList[i].IMCreateTime = $frameworkFormatDate(imServer.lastUserList[i].IMCreateTime, "MM-dd hh:mm");
        ...
        return {
            "init": function (fnDisconnected,$scope) {
                ...
                if (!imServer.isIMLoaded) {
                    $frameworkLoadJs(SignalrUrl + "/hubs").then(
                        function (data) {
                            if (data == "success") {
  ...
  .factory('lrmOrderInfos',[
    '$frameworkHttp', 'ApiUrl', '$frameworkTopAlert', '$frameworkFormatDate',
    function ($frameworkHttp, ApiUrl, $frameworkTopAlert, $frameworkFormatDate) {
      //客户列表
      var orderInfoList = {
        records: 0,
        page: 1,
      ...
      function translateData(data, obj) {
        for (var i in data) {
          ...
          data[i].f_CreateDate = $frameworkFormatDate(item.f_CreateDate, 'yyyy-MM-dd');
          data[i].f_ModifyDate = $frameworkFormatDate(item.f_ModifyDate, 'yyyy-MM-dd');
          data[i].f_PaymentDate =$frameworkFormatDate(item.f_PaymentDate, 'yyyy-MM-dd');
          data[i].f_OrderDate = $frameworkFormatDate(item.f_OrderDate, 'yyyy-MM-dd');
          obj.push(data[i]);
        }
      };
      //获取数据
  ...
  .factory('lrmReceivableReports',[
    '$frameworkHttp', '$frameworkFormatDate', '$frameworkTopAlert', 'ApiUrl',
    function ($frameworkHttp, $frameworkFormatDate, $frameworkTopAlert, ApiUrl) {
      //客户列表
      ...
      function translateData(data, obj) {
        for (var i in data) {
          ...
          data[i].f_CreateDate = $frameworkFormatDate(item.f_CreateDate, 'yyyy-MM-dd');
          data[i].f_PaymentTime = $frameworkFormatDate(item.f_PaymentTime, 'yyyy-MM-dd');
          obj.push(data[i]);
        }
      };
      //获取数据
  ...
  //我的账号信息
  .factory('UserInfo', ['$frameworkLocals', function ($frameworkLocals) {
    var userInfo = {
        userid: "",
        account: "",
        password: "",
    ...
    return {
        get: function () {
            var newUserInfo = $frameworkLocals.getObject('userInfo');
        ...
        set: function (data, token, isLogin) {
            ...
            $frameworkLocals.setObject('userInfo', userInfo);
        },
        clear: function ()
        {
            userInfo.isLogin = false;
            userInfo.token = "";
            $frameworkLocals.setObject('userInfo', {});
        }
    };
}])
  //行政区域数据
  .factory('AreaInfo', ['$frameworkHttp', 'ApiUrl', function ($frameworkHttp, ApiUrl) {
    var areaInfo = {};
    return {
      init: function () {
          $frameworkHttp.post({
  ...
  //基础信息
  .factory('lrmBaseInfo', ['$frameworkHttp', 'ApiUrl', function ($frameworkHttp, ApiUrl) {
    var baseInfo ={
      userListInfo:[]//用户列表
    };
    return{
