angular.module('starter.services', [])
  ...
  //post 请求
  .factory('$frameworkHttp', [
    '$rootScope',
    '$base64',
    '$http',
    '$frameworkTopAlert',
    'UserInfo',
  function ($rootScope, $base64, $http, $frameworkTopAlert, UserInfo) {
      return {
          post: httpPost
      };
   ...
   //本地存储数据
  .factory('$frameworkLocals', ['$window', function ($window) {
      return {
          //存储单个属性
  ...
  //弹层
  .factory('$frameworkPopup', ['$rootScope', '$ionicPopup', function ($rootScope, $ionicPopup) {
      return {
          confirm: confirm
      }
  ...
  //触发下拉刷新
  .service('$frameworkTriggerRefresh', ['$timeout', '$ionicScrollDelegate', function ($timeout, $ionicScrollDelegate) {
      /**
  ...
  //格式化时间
  .factory('$frameworkFormatDate', function () {
  ...
  //页面模型
  .factory('$frameworkPageModal', ['$ionicModal', function ($ionicModal) {
  ...
  //选择页面模型
  .factory('$frameworkSelectModal', ['$rootScope', '$ionicModal', function ($rootScope, $ionicModal) {
  ...
  //判断数据是否完整
  .factory('$frameworkDataIsAll', function () {
    return {
  ...
  //获取数据字典的数据
  .factory('$frameworkGetDataItem', ['$frameworkHttp', 'ApiUrl', function ($frameworkHttp, ApiUrl) {
    return function (ops) {
        $frameworkHttp.post({
        ...
        "success": function (data) {
          ...
          for(var i in data.result) {
            ...
            lsdata[item.f_ItemValue] = item;
            lsdata[item.f_ItemValue].bgColor = bgColorList[i];
          }
          ops.callback(lsdata);
        }
      });
  ...
  //工作流表单转换
  .factory('$frameworkFormDesign', function () {
        return {
...
//动态加载js
.factory('$frameworkLoadJs', ['$http', '$q', function ($http, $q) {
    return function (url) {
