        ...
        public object GetAuditList([FromBody]JObject json)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        //审核
                        strWhere += string.Format(@"   select  distinct a.id from bis_safetychange a  where a.flowdept like'%{0}%' and a.flowrolename like '%{1}%' and  a.isapplyover =1 and  a.iscommit =1 and  a.isaccpcommit =1  union", deptId, str);
                        //审核
                        strWhere += string.Format(@"   select  distinct a.id from bis_safetychange a  where a.flowdept like'%{0}%' and a.flowrolename like '%{1}%' and  a.isapplyover =0 and  a.iscommit =1  union", deptId, str);
                    }
