...
namespace ERCHTMS.Service.HiddenTroubleManage
{
    ...
    /// 描 述：流程配置条件附加表
    /// </summary>
    public class WfConditionAddtionService : RepositoryFactory<WfConditionAddtionEntity>, WfConditionAddtionIService
    {
        #region 获取数据
        /// <summary>
        /// 获取列表
        ...
        public IEnumerable<WfConditionAddtionEntity> GetList(string queryJson)
        {
            ...
            {
                return this.BaseRepository().IQueryable().Where(p => p.WFCONDITIONID == queryJson).ToList();
            }
            else
            {
