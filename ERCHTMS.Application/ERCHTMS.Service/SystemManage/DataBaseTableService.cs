    ...
    public class DataBaseTableService : RepositoryFactory, IDataBaseTableService
    {
        ...
        public DataTable GetTableList(string dataBaseLinkId, string tableName)
        {
            ...
            if (dataBaseLinkEntity != null)
            {
                ...
                {
                    strSql.Append(@"select a.table_name as name,a.num_rows as sumrows , a.last_analyzed as updatetime , d.column_name as pk ,e.comments as tdescription ,0 as reserved  from  user_tables a
                    ...
                    if (!string.IsNullOrEmpty(tableName))
                    {
                        strSql.Append(" and a.table_name like '%" + tableName.Trim().ToUpper() + "%'");
                    }
                ...
                {
                    strSql.Append(@"select DISTINCT a.table_name as name,table_rows as sumrows,update_time as updatetime,column_name as pk,TABLE_comment as tdescription,data_length as  reserved from information_schema.TABLES a left join information_schema.`COLUMNS` b on a.table_name=b.table_name where a.TABLE_SCHEMA='" + dataBaseLinkEntity.DBName + "' and column_key='PRI'");
                    if (!string.IsNullOrEmpty(tableName))
                    {
                        strSql.Append(" and a.table_name like '" + tableName + "%'");
                    }
                    strSql.Append(" order by a.table_name");
                }
        ...
        public DataTable GetTableDataList(string dataBaseLinkId, string tableName, string switchWhere, string logic, string keyword, Pagination pagination)
        {
            ...
            if (dataBaseLinkEntity != null)
            {
                ...
                if (dataBaseLinkEntity.DbType == "SqlServer")
                {
                    return this.BaseRepository(dataBaseLinkEntity.DbConnection, DatabaseType.SqlServer).FindTable(strSql.ToString(), parameter.ToArray(), pagination);
                }
                else if (dataBaseLinkEntity.DbType == "Oracle")
                {
                    return this.BaseRepository(dataBaseLinkEntity.DbConnection, DatabaseType.Oracle).FindTable(strSql.ToString(), parameter.ToArray(), pagination);
                }
                else if (dataBaseLinkEntity.DbType == "MySql")
                {
                    return this.BaseRepository(dataBaseLinkEntity.DbConnection, DatabaseType.MySql).FindTable(strSql.ToString(), parameter.ToArray(), pagination);
                }
            }
            return null;
        }
        ...
        public void SaveForm(string dataBaseLinkId, string tableName, string tableDescription, IEnumerable<DataBaseTableFieldEntity> fieldList)
        {
            ...
            if (dataBaseLinkEntity != null)
            {
                  ...
                  {
                      strSql.AppendFormat("select count(1) from user_tables where table_name='{0}'", tableName.ToUpper());
                  }
                  if (dataBaseLinkEntity.DbType == "SqlServer")
                  {
                      strSql.AppendFormat("select count(1) from sysobjects where name='{0}'", tableName);
                  }
                  if (dataBaseLinkEntity.DbType == "MySql")
                  {
                      strSql.AppendFormat("select count(1) from information_schema.`TABLES` where TABLE_NAME='{0}'", tableName);
                  }
                  count = this.BaseRepository(dataBaseLinkEntity.DbConnection, dataBaseLinkEntity.DbType).FindObject(strSql.ToString()).ToString();
                  if (count == "0")
                  {
                         strSql.Length = 0;
                          strSql.AppendFormat("create table {0} (", tableName.ToUpper());
                          foreach (DataBaseTableFieldEntity entity in fieldList)
                          {
                              strSql.AppendFormat("{0} {1}", entity.column_name, entity.datatype);
                             
                              if (!string.IsNullOrEmpty(entity.defaults))
                              {
                                  if (ValidateUtil.IsNumberType(entity.datatype))
                                  {
                                       strSql.AppendFormat(" default {0}", entity.defaults);
                                  }
                                  else
                                  {
                                      strSql.AppendFormat(" ({0}) default '{1}'",entity.length_data,entity.defaults);
                                  }
                              }
                              if (!string.IsNullOrEmpty(entity.key))
                              {
                                  strSql.AppendFormat(" {0}", "primary key");
                              }
                              if (string.IsNullOrEmpty(entity.isnullable))
                              {
                                  strSql.AppendFormat(" {0}", "not null");
                              }
                              strSql.Append(",");
                          }
                          this.BaseRepository(dataBaseLinkEntity.DbConnection, dataBaseLinkEntity.DbType).ExecuteBySql(strSql.ToString().TrimEnd(',') + ")");
                      }
             }
        ...
        {
            DataBaseLinkEntity dataBaseLinkEntity = dataBaseLinkService.GetEntity(dataBaseLinkId);
            this.BaseRepository(dataBaseLinkEntity.DbConnection, dataBaseLinkEntity.DbType).ExecuteBySql(string.Format("drop table {0}",tableName));
        }
        #endregion
    }
}
