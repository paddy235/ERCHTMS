...
namespace ERCHTMS.Service.SystemManage
{
    ...
    public class DataBaseTableService : RepositoryFactory, IDataBaseTableService
    {
        ...
        {
            DatabaseType dbtype = DbHelper.DbType;
            StringBuilder strSql = new StringBuilder();
            ...
            foreach (DataBaseTableFieldEntity tf in fieldList)
            {
                strSql.AppendFormat("{0} {1}({2}) {3} {4}",tf.column_name,tf.datatype,tf.length_data,tf.isnullable=="1"?"not null":"null",tf.key=="0"?"":"primary key");
            }
