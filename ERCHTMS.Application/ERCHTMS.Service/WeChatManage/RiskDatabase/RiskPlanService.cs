...
namespace ERCHTMS.Service.RiskDatabase
{
    ...
    public class RiskPlanService : RepositoryFactory<RiskPlanEntity>, RiskPlanIService
    {
        ...
        /// <param name="startDate">计划开始时间</param>
        /// <returns></returns>
        public int GetRiskNumbers(string areaIds,string startDate)
        {
            string sql = string.Format("select count(1) from BIS_RISKASSESS where createdate<to_date('{0}','yyyy-mm-dd hh24:mi:ss') and status=1 and deletemark=0 and EnabledMark=0 and districtid in('{1}')", DateTime.Parse(startDate).ToString("yyyy-MM-dd 00:00:01"), areaIds.Replace(",", "','"));
        ...
        {
            int count = 0;
            string sql = string.Format("select planid from BIS_RISKPPLANDATA where datatype=0 and userid='{0}'", user.Account);
            DataTable dt = this.BaseRepository().FindTable(sql);
            if (dt.Rows.Count > 0)
            {
                StringBuilder sb = new StringBuilder();
                ...
                sql = "select count(1) from BIS_RISKPLAN where  status=0 ";
                //当前用户待辨识或评估的计划
                if (mode == 0)
                {
                ...
                else
                {
                    ...
                    sql += string.Format(" or id in(select planid from BIS_RISKPPLANDATA where deptcode='{0}'))", user.DeptCode);
        ...
        public bool SetComplate(string planId,string areaIds)
        {
            ...
            try
            {
                string sql = string.Format("update BIS_RISKPLAN set status=1 where id='{0}'", planId);
                if (this.BaseRepository().ExecuteBySql(sql) > 0)
                {
                    string id = Guid.NewGuid().ToString();
                    sql = string.Format(@"insert into BIS_RISKHISTORY(
                    id,areaid,areaname,dangersource,way,itema,itemb,itemc,itemr,grade,deptcode,deptname,postid,postname,createuserid,createdate,createusername,createuserdeptcode,createuserorgcode,status,gradeval,result,accidentname,harmtype,risktype,DeleteMark,planid,state,districtid,districtname,areacode,newplanid)
select  '{0}' || rownum,areaid,t.areaname,t.dangersource,way,itema,itemb,itemc,itemr,grade,deptcode,deptname,postid,postname,createuserid,createdate,createusername,createuserdeptcode,createuserorgcode,status,gradeval,result,accidentname,harmtype,risktype,DeleteMark,planid,state,districtid,districtname,areacode,'{1}' from bis_riskassess t
where (t.districtid in('{2}') and status=1) or (planid='{1}' and status=2) ", id, planId, areaIds.Replace(",","','"));
                    this.BaseRepository().ExecuteBySql(sql);
                    DataTable dt = this.BaseRepository().FindTable(string.Format("select '{0}' || rownum,id from bis_riskassess where (districtid in('{2}') and status=1) or (planid='{1}' and status=2) and id in(select riskid from BIS_MEASURES)", id, planId, areaIds.Replace(",", "','")));
                    foreach(DataRow dr in dt.Rows)
                    {
                        id = Guid.NewGuid().ToString();
                        sql = string.Format("insert into BIS_MEASURES(id,content,riskid,areacode) select '{0}' || rownum,content,'{1}',areacode from BIS_MEASURES where riskid='{2}' ", id, dr[0].ToString(), dr[1].ToString());
                        this.BaseRepository().ExecuteBySql(sql);
                    }
