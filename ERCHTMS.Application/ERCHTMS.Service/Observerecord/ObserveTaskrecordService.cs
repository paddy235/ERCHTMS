...
{
    public class ObserveTaskrecordService : RepositoryFactory<ObserveTaskrecordEntity>, ObserveTaskrecordIService
    {
        #region 获取数据
        /// <summary>
        /// 获取列表
        ...
        public IEnumerable<ObserveTaskrecordEntity> GetList()
        {
        ...
        /// <returns></returns>
        public ObserveTaskrecordEntity GetEntity(string keyValue)
        {
...
from (select d.itemvalue,d.itemname,d.sortcode from base_dataitemdetail d where d.itemid in (select itemid from base_dataitem t where t.itemcode='ObsType')) d
    left join(select s.observetype,s.observetypename,
        ...
        {
            string sql = string.Format("select t.id,t.isallsafety,t.existproblemcode,t.observetype  from BIS_OBSERVETaskCATEGORY t where t.recordid='{0}'", keyValue);
            return this.BaseRepository().FindTable(sql);
        }
        /// <summary>
        ...
        {
            string sql = string.Format(@"select count(1) from bis_observeTaskrecord t where t.obsplanid like'{0}%' and t.obsplanfjid like'{1}%'", planid, planfjid);
        ...
        public void RemoveForm(string keyValue)
        {
            ...
            try
            {
                string sql = string.Format("delete bis_observeTasksafety t where t.recordid='{0}'", keyValue);
                res.ExecuteBySql(sql);
                sql = string.Format("delete bis_observeTaskcategory t where t.recordid='{0}'", keyValue);
                res.ExecuteBySql(sql);
                ObserveTaskrecordEntity obsentity = res.FindEntity<ObserveTaskrecordEntity>(keyValue);
        ...
        /// <returns></returns>
        public void SaveForm(string keyValue, ObserveTaskrecordEntity entity, List<ObserveTaskcategoryEntity> listCategory, List<ObserveTasksafetyEntity> listSafety)
        {
            var res = DbFactory.Base().BeginTrans();
            try
            {
                ...
                if (!string.IsNullOrEmpty(keyValue))
                {
                    ...
                    if (newEntity != null)
                    {
                        ...
                        res.Update<ObserveTaskrecordEntity>(entity);
                    }
                    else
                    {
                        entity.Create();
                        res.Insert<ObserveTaskrecordEntity>(entity);
                    }
                ...
                res.ExecuteBySql(string.Format("delete from bis_observeTaskcategory t where t.recordid='{0}'", entity.ID));
                res.ExecuteBySql(string.Format("delete from bis_observeTasksafety t where t.recordid='{0}'", entity.ID));
                ...
                res.Insert<ObserveTaskcategoryEntity>(listCategory);
                res.Insert<ObserveTasksafetyEntity>(listSafety);
                res.Commit();
            }
