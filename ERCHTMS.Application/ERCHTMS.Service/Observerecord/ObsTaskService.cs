...
{
   public  class ObsTaskService : RepositoryFactory<ObsTaskEntity>, IObsTaskService
    {
        #region 获取数据
        /// <summary>
        /// 获取列表
        ...
        /// <summary>
        /// 根据观察计划Id与任务分解Id获取相应信息--此处查询的是发布后的数据--BIS_OBSTASK_FB
        /// </summary>
        ...
        public DataTable GetPlanById(string PlanId, string PlanFjId, string month)
        {
            string sql = string.Format(@"select t.workname||'_'||p.workname WorkName,t.planarea WorkArea,t.planyear,
                                      ...
                                      from bis_obsTask_fb
t left join bis_obsTaskwork p on p.planid = t.id
        ...
        public void RemoveForm(string keyValue)
        {
            ...
            try
            {
                string subSql = string.Format(@"delete from BIS_OBSTASKWORK t where  t.planid='{0}'", keyValue);
        ...
        public void SaveForm(string keyValue, ObsTaskEntity entity)
        {
            ...
            if (!string.IsNullOrEmpty(keyValue))
            {
                ...
                ObsTaskEntity old = this.BaseRepository().FindEntity(keyValue);
                if (old == null)
                {
                    entity.Create();
        ...
        public bool CommitEhsData(Operator currUser)
        {
            ...
            try
            {
                var oldTable = "BIS_OBSTASK";
                var tableName = "BIS_OBSTASK_COMMITEHS";
                var oldFeedBackTable = "BIS_OBSTASKFEEDBACK";
                var newFeddBackTable = "BIS_OBSTASKFEEDBACK_EHS";
                if (currUser.RoleName.Contains("公司级") && currUser.RoleName.Contains("安全管理员"))
                {
                    oldTable = "BIS_OBSTASK_FB";
                    tableName = "BIS_OBSTASK_TZ";
                    ...
                    sbSql.Append(string.Format(" UPDATE BIS_OBSTASK_FB SET ISCOMMIT='1',ISPUBLIC='1'"));
                    res.ExecuteBySql(sbSql.ToString());
                    //将发布状态同步到部门表中
                    string updateSql1 = string.Format(@"UPDATE BIS_OBSTASK_COMMITEHS SET ISPUBLIC='1' WHERE OLDPLANID IN(SELECT OLDPLANID FROM BIS_OBSTASK_FB) OR ID IN(SELECT OLDPLANID FROM BIS_OBSTASK_FB)");
                    res.ExecuteBySql(updateSql1);
                    string updateSql = string.Format(@"UPDATE BIS_OBSTASK SET ISPUBLIC='1' WHERE ID IN(SELECT OLDPLANID FROM BIS_OBSTASK_COMMITEHS)");
                ...
                else
                {
                    ...
                    {
                        oldTable = "BIS_OBSTASK_COMMITEHS";
                        tableName = "BIS_OBSTASK_FB";
                        oldFeedBackTable = "BIS_OBSTASKFEEDBACK_EHS";
                        newFeddBackTable = "BIS_OBSTASKFEEDBACK_FB";
                        ...
                        //先将已经提交的数据状态更新到部门表中
                        string update = string.Format(@"UPDATE BIS_OBSTASK SET ISEMSCOMMIT='1' WHERE ID IN(SELECT OLDPLANID FROM BIS_OBSTASK_COMMITEHS)");
                ...
                subsbSql.Append("INSERT INTO BIS_OBSTASKWORK(ID,CREATEUSERID,CREATEUSERDEPTCODE,CREATEUSERORGCODE,CREATEDATE,CREATEUSERNAME,MODIFYDATE,MODIFYUSERID,MODIFYUSERNAME,OBSPERSON,");
                ...
                subsbSql.Append(string.Format("OBSPERSONID,RISKLEVEL,OBSNUM,OBSMONTH,(PLANID||'-'||'" + newId + "'),WORKNAME,REMARK,OBSNUMTEXT,{2} FROM BIS_OBSTASKWORK WHERE PLANID IN(SELECT ID FROM {1} T WHERE 1=1 {0})", where, oldTable, strCase));
        ...
        public void InsertImportData(List<ObsTaskEntity> obsplan, List<ObsTaskworkEntity> obsplanwork)
        {
            ...
            try
            {
                ...
                res.Insert<ObsTaskEntity>(obsplan);
                res.Insert<ObsTaskworkEntity>(obsplanwork);
                res.Commit();
            }
        ...
        public void InsertImportData(List<ObsTaskEHSEntity> obsplan, List<ObsTaskworkEntity> obsplanwork)
        {
            ...
            try
            {
                ...
                res.Insert<ObsTaskEHSEntity>(obsplan);
                res.Insert<ObsTaskworkEntity>(obsplanwork);
                res.Commit();
            }
            catch (System.Exception)
        ...
        public bool CopyHistoryData(Operator currUser, string oldYear, string newYear)
        {
            ...
            try
            {
                //部门用户复制本部门数据--EHS部复制EHS部数据（EHS数据在BIS_OBSTASK_COMMITEHS）
                var tableName = "BIS_OBSTASK";
                var strCase = "ID";

                if (currUser.RoleName.Contains("厂级部门"))
                {
                    tableName = "BIS_OBSTASK_COMMITEHS";
                    strCase = "CASE WHEN OLDWORKID IS NOT NULL THEN OLDWORKID ELSE ID END ";
                }
                string newId = Guid.NewGuid().ToString();
                StringBuilder sbSql = new StringBuilder();
                ...
                subsbSql.Append("OBSPERSONID,RISKLEVEL,OBSNUM,OBSMONTH,PLANID,WORKNAME,REMARK,OBSNUMTEXT,OLDWORKID)");
                subsbSql.Append("SELECT (ID||'-'||'" + newId + "'),CREATEUSERID,CREATEUSERDEPTCODE,CREATEUSERORGCODE,CREATEDATE,CREATEUSERNAME,MODIFYDATE,MODIFYUSERID,MODIFYUSERNAME,OBSPERSON,");
                subsbSql.Append(string.Format("OBSPERSONID,RISKLEVEL,OBSNUM,OBSMONTH,(PLANID||'-'||'" + newId + "'),WORKNAME,REMARK,OBSNUMTEXT,{0} FROM BIS_OBSTASKWORK WHERE PLANID ", strCase));
        ...
        {
            IObsTaskworkService work = new ObsTaskworkService();
            var workentity = work.GetEntity(planfjid);

            string sql = string.Format(@"SELECT * FROM BIS_OBSTASKWORK T WHERE T.PLANID LIKE'{0}%' AND T.ID LIKE '{1}%'", planid, planfjid);
            var dt = this.BaseRepository().FindTable(sql);
            if (dt.Rows.Count > 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    string update = string.Format(@"UPDATE BIS_OBSTASKWORK T  SET T.OBSMONTH='{0}' WHERE T.ID='{1}'", workentity.ObsMonth, dt.Rows[i]["id"].ToString());
