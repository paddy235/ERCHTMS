...
namespace ERCHTMS.Service.AuthorizeManage
{
    ...
    public class AuthorizeService : RepositoryFactory, IAuthorizeService
    {
        ...
        public IEnumerable<ModuleEntity> GetModuleList(string userId)
        {
            ...
            return this.BaseRepository().FindList<ModuleEntity>(strSql.ToString(), new DbParameter[] { DbParameters.CreateDbParameter("@UserId", userId) });
        }
        /// <summary>
        /// 获取授权功能按钮
        /// </summary>
        ...
        public IEnumerable<ModuleButtonEntity> GetModuleButtonList(string userId)
        {
            ...
            return this.BaseRepository().FindList<ModuleButtonEntity>(strSql.ToString(), new DbParameter[] { DbParameters.CreateDbParameter("@UserId", userId) });
        }
            ...
            strSql.AppendFormat(@"SELECT encode,moduleid
                            ...
                            WHERE   ModuleButtonId IN (
                                    ...
                                    WHERE   ItemType = 2
                                            AND ( ObjectId IN ({0}) )
                                            OR ObjectId = @UserId ) Order By SortCode", sb.ToString().Substring(0, sb.Length - 1));
            return this.BaseRepository().FindTable(strSql.ToString(), new DbParameter[] { DbParameters.CreateDbParameter("@UserId", userId) });
        }
        /// <summary>
        /// 获取授权功能视图
        /// </summary>
        ...
        public IEnumerable<ModuleColumnEntity> GetModuleColumnList(string userId)
        {
            ...
            return this.BaseRepository().FindList<ModuleColumnEntity>(strSql.ToString(), new DbParameter[] { DbParameters.CreateDbParameter("@UserId", userId) });
        }
        /// <summary>
        /// 获取授权功能Url、操作Url
        /// </summary>
        ...
        public IEnumerable<AuthorizeUrlModel> GetUrlList(string userId)
        {
            ...
            return this.BaseRepository().FindList<AuthorizeUrlModel>(strSql.ToString(), new DbParameter[] { DbParameters.CreateDbParameter("@UserId", userId) });
        }
        /// <summary>
        /// 获取关联用户关系
        /// </summary>
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                switch (authorizeType)
                {
                    case "1":
                        ...
                        break;
                    case "2":
                        result = deptCode + "='" + user.DeptCode + "'";//查询本部门的数据
                        break;
                    case "3":
                        result = deptCode + " like '" + user.DeptCode + "%'";//查询部门及下属部门的数据
                        break;
                    case "4":
                    ...
                    case "5": //全部
                        string nature = "厂级";
                        if (user.RoleName.Contains("集团"))
                        {
                            nature = "省级";
                            result = orgCode + "='" + user.OrganizeCode + "' or " + orgCode + string.Format(" in(select encode from BASE_DEPARTMENT d where d.deptcode like '{0}%' and d.nature='{1}' and d.description is null)", user.NewDeptCode, nature);//查询所有下属省级的数据
                        }
                        else if (user.RoleName.Contains("省级"))
                        {
                            nature = "厂级";
                            result = "("+orgCode + "='" + user.OrganizeCode + "' or " + orgCode + string.Format(" in(select encode from BASE_DEPARTMENT d where d.deptcode like '{0}%' and d.nature='{1}' and d.description is null))", user.NewDeptCode, nature);//查询所有下属电厂的数据
                        }
                        else
                        {
                            result = orgCode + "='" + user.OrganizeCode + "'";//查询本机构的数据
        ...
        /// </summary>
        /// <param name="user">当前用户</param>
        /// <param name="moduleEncode">模块编码</param>
        ...
        public bool HasOperAuthorityEncode(Operator user, string moduleEncode, string enCode)
        {
            ...
            string userId = user.UserId;
            string sql = @"select count(1) from BASE_AUTHORIZEDATA t where resourceid in (select moduleid from BASE_MODULE where encode=@moduleencode) and t.objectid in(select objectid from BASE_USERRELATION where userid=@userId) and itemcode=@enCode";
            string count = this.BaseRepository().FindObject(sql, new DbParameter[] {DbParameters.CreateDbParameter("@moduleencode", moduleEncode), DbParameters.CreateDbParameter("@userId", userId), DbParameters.CreateDbParameter("@enCode", enCode) }).ToString();
        ...
        public bool HasOperAuthority(Operator user, string moduleId, string enCode)
        {
            ...
            string userId = user.UserId;
            string sql = @"select count(1) from BASE_AUTHORIZEDATA t where resourceid=@moduleId and t.objectid in(select objectid from BASE_USERRELATION t where userid=@userId) and itemcode=@enCode";
            string count = this.BaseRepository().FindObject(sql, new DbParameter[] {DbParameters.CreateDbParameter("@moduleId", moduleId),  DbParameters.CreateDbParameter("@userId", userId), DbParameters.CreateDbParameter("@enCode", enCode) }).ToString();
            return count == "0" ? false : true;
        ...
        /// </summary>
        /// <param name="user">当前用户</param>
        /// <param name="moduleEncode">模块Id</param>
        ///  <param name="enCode">功能编号，如add(新增),edit(修改),delete(删除),export(导出)等</param>
        /// <returns>1:本人，2：本部门，3：本子部门，4：本机构，5：全部</returns>
        public string GetOperAuthorzeTypeEncode(Operator user, string moduleEncode, string enCode)
        {
            ...
            else
            {
                string userId = user.UserId;
                string sql = @"select max(AUTHORIZEType) from BASE_AUTHORIZEDATA t where t.objectid in(select objectid from BASE_USERRELATION t
                where userid=@userId) and resourceid in (select moduleid from BASE_MODULE where encode=@moduleencode) and itemcode=@enCode";
                object obj = this.BaseRepository().FindObject(sql, new DbParameter[] { DbParameters.CreateDbParameter("@userId", userId), DbParameters.CreateDbParameter("@moduleencode", moduleEncode), DbParameters.CreateDbParameter("@enCode", enCode) });
                return obj == DBNull.Value || obj == null ? "" : obj.ToString();
            }
        }
        /// <summary>
