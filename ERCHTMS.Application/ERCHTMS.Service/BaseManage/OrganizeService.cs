...
namespace ERCHTMS.Service.BaseManage
{
    ...
    public class OrganizeService : RepositoryFactory<OrganizeEntity>, IOrganizeService
    {
        ...
        public DataTable GetDataTable(string sql, string connString = "ConnString")
        {
            return BaseRepository(connString).FindTable(sql);
        }
        /// <summary>
        /// 机构列表
        /// </summary>
        ...
        public DataTable GetDTList()
        {
            ...
            return this.BaseRepository().FindTable(string.Format("select encode,fullname,organizeid as orgid  from BASE_DEPARTMENT where nature='厂级' and deptcode like '{0}%'", user.OrganizeCode));
        }
        /// <summary>
        /// 根据部门编码获取机构编码
        /// </summary>
        ...
        {
            object obj = BaseRepository().FindObject(string.Format("select t.encode from BASE_DEPARTMENT t where t.organizeid=(select d.organizeid from base_department d where d.encode='{0}')", deptCode));
