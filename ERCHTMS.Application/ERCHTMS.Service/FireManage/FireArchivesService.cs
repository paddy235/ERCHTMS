...
namespace ERCHTMS.Service.FireManage
{
    ...
    /// 描 述：消防档案
    /// </summary>
    public class FireArchivesService : RepositoryFactory<FireArchivesEntity>, FireArchivesIService
    {
        #region 获取数据
        /// <summary>
        /// 获取列表
        ...
        public DataTable GetPageList(Pagination pagination, string queryJson)
        {
            ...
            if (queryJson != null && queryJson != "")
            {
                ...
                if (!queryParam["state"].IsEmpty())
                {
                    ...
                    {
                        //消防档案类型
                        if (!queryParam["ArchivesType"].IsEmpty())
                        {
                            pagination.conditionJson += string.Format(" and ArchivesType='{0}'", queryParam["ArchivesType"].ToString());
                        }
                    }
                    else
                    {
        ...
        public IEnumerable<FireArchivesEntity> GetList(string queryJson)
        {
        ...
        /// <returns></returns>
        public FireArchivesEntity GetEntity(string keyValue)
        {
        ...
        /// <returns></returns>
        public void SaveForm(string keyValue, FireArchivesEntity entity)
        {
            entity.Id = keyValue;
            if (!string.IsNullOrEmpty(keyValue))
            {
                FireArchivesEntity fe = this.BaseRepository().FindEntity(keyValue);
                if (fe == null)
                {
                    entity.Create();
