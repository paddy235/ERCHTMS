...
namespace ERCHTMS.Busines.PersonManage
{
    ...
    /// 1.4版及以上版本权限下发
    /// </summary>
    public class PersonNewBLL
    {
        private PersonNewIService service = new PersonNewService();
        ...
        /// 删除设备中对应出入权限配置
        /// </summary>
        public void DeleteUserlimits(List<TemporaryUserEntity> list, string baseUrl, string Key, string Signature)
        {
            try
            {
                service.DelEquipmentRecord(list, baseUrl, Key, Signature);
            }
            catch (Exception)
            {
                throw;
