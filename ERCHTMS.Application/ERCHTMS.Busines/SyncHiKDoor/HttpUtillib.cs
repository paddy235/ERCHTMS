...
namespace ERCHTMS.Busines.SyncHiKDoor
{
    public class HttpUtillib
    {
        ...
        public static byte[] HttpPost(string uri, string body, int timeout)
        {
            ...
            // build web request object
            string url = (_isHttps ? "https://" : "http://") + $"{_ip}:{_port}{uri}";
            // StringBuilder sb = new StringBuilder();
            //sb.Append(url);
            // 创建POST请求
            ...
            if (!string.IsNullOrWhiteSpace(body))
            {
                ...
                catch (WebException e)
                {
                    ...
                    return System.Text.Encoding.UTF8.GetBytes(e.Message + $"。Api地址：{url},传参：{body}\r\n");
                }
            }
