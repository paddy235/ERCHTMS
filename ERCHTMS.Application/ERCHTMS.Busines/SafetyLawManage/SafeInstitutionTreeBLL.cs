...
{
    public class SafeInstitutionTreeBLL : BaseBLL<SafeInstitutionTreeEntity>
    {
        /// <summary>
        /// 查询
        /// </summary>
        ...
        public override DataTable GetList(Pagination pagination, string queryJson)
        {
            ...
            pagination.p_kid = "id";
            pagination.p_tablename = @"BIS_SafeInstitutionTree";
        ...
        {
            SafeInstitutionBLL safeinstitutionbll = new SafeInstitutionBLL();
            string sql = string.Format(" lawtypeid in(select id from BIS_SafeInstitutionTree start with id='{0}' connect by  prior id = parentid)", keyValue);
            new DepartmentBLL().ExecuteSql("delete from BIS_SAFEINSTITUTION where " + sql);
            var l = this.GetList(string.Format(" and id in(select id from BIS_SafeInstitutionTree start with id='{0}' connect by  prior id = parentid)", keyValue)).ToList();
            base.RemoveForm(l);
        }
    }
}
