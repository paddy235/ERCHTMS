...
function goTrain(obj) {
    ...
    if (top.way == "1") {
        ...
        if (top.isTrainAdmin == "1") {
            var html = "<div id='divTrain' style='padding:10px;'><div style='color:red;'>系统检测到您拥有多个角色,请选择一个角色身份进行登录.</div>选择角色：<select id='selRole' class='form-control' style='width:150px;display:inline'><option value=''>请选择</option><option value='1'>培训管理员</option><option value='0'>学员</option></select></div>";
            var win = dialogContent({
                title: "跳转至教育培训",
                id: "train",
                btns: 1,
                btn: ["确认"],
...
(function ($) {
    tablist = {
        createTab: function (dataId, dataUrl, menuName) {
    ...
    $.tab = {
                ...
                if ($(this).parents('.menuTab').next('.menuTab').size()) {
                    ...
                    $('.mainContent .LRADMS_iframe').each(function () {
                        if ($(this).data('id') == activeId) {
                            var dataId = $(this).attr('id').replace("iframe", "");
                            top.$.cookie('currentmoduleId', dataId, { path: "/" });
                            $(this).show().siblings('.LRADMS_iframe').hide();
                            return false;
                        }
        ...
        init: function () {
            ...
            $('.menuItem1').on('mouseup', top.$.tab.addTab1);
    ...
    $.index = {
        load: function () {
            ...
            //加载首页 
            var homeUrl = contentPath + "/Home/Desktop";

            $("#Desktop").attr("src", homeUrl);
        },
        ...
        loadMenu: function () {
                ...
                if (row.ParentId == "0") {
                    if (row.UrlAddress == null) {
                        ...
                        if ($.trim(row.UrlAddress).length < 1) {
                            ...
                            var url = row.UrlAddress.indexOf("?") >= 0 ? row.UrlAddress + "&mId=" + row.ModuleId : row.UrlAddress + "?mId=" + row.ModuleId;
                            if (row.UrlAddress.indexOf("http") >= 0 && row.FullName.indexOf("安全教育培训") < 0) {
                                _html += '<li><a  href="' + url + '" data-id="' + row.ModuleId + '" class="menuItem"><i class="fa ' + row.Icon + '"></i><span>' + row.FullName + '</span></a><em></em></li>';
                            }
                            else {
                                //安全教育培训专用链接
                                if (row.FullName.indexOf("安全教育培训") >= 0) {
                                    _html += '<li><a  id="atrain" url="' + url + '" onclick="goTrain(this)" style="cursor:pointer;"><i class="fa ' + row.Icon + '"></i><span>' + row.FullName + '</span></a><em></em></li>';
                                }
                                else {
                                    _html += '<li><a data-href="' + contentPath + url + '" data-id="' + row.ModuleId + '" class="menuItem1"><i class="fa ' + row.Icon + '"></i><span>' + row.FullName + '</span></a><em></em></li>';
                                }
                    ...
                    if (subchildNodes.length > 0) {
                        ...
                        if (row.UrlAddress == null) {
                            ...
                            if ($.trim(row.UrlAddress).length < 1) {
                                ...
                                if (row.UrlAddress.indexOf("http") >= 0) {
                                    _childNodeshtml += '<a href="' + url + '" class="menuItem"><i class="fa ' + row.Icon + '"></i><span>' + row.FullName + '</span><i class="fa fa-angle-left pull-right"></i></a>';
                                } else {
                                    _childNodeshtml += '<a href="' + contentPath + url + '" class="menuItem"><i class="fa ' + row.Icon + '"></i><span>' + row.FullName + '</span><i class="fa fa-angle-left pull-right"></i></a>';
                                }
                            }
                        }
                        ...
                        $.each(subchildNodes, function (i) {
                            ...
                            if (subrow.UrlAddress == null) {
                                _childNodeshtml += '<li><a class="menuItem" data-id="' + subrow.ModuleId + '" href="#"><i class="fa ' + subrow.Icon + '"></i><span>' + subrow.FullName + '</span></a></li>';
                            } else {
                                var url = subrow.UrlAddress.indexOf("?") >= 0 ? subrow.UrlAddress + "&mId=" + subrow.ModuleId : subrow.UrlAddress + "?mId=" + subrow.ModuleId;
                                if (subrow.UrlAddress.indexOf("http") >= 0) {
                                    _childNodeshtml += '<li><a class="menuItem" data-id="' + subrow.ModuleId + '" href="' + url + '"><i class="fa ' + subrow.Icon + '"></i><span>' + subrow.FullName + '</span></a></li>';
                                } else {
                                    _childNodeshtml += '<li><a class="menuItem" data-id="' + subrow.ModuleId + '" href="' + contentPath + url + '"><i class="fa ' + subrow.Icon + '"></i><span>' + subrow.FullName + '</span></a></li>';
                                }
                            }
                        });
                        _childNodeshtml += '</ul>';
                        ...
                        if (row.UrlAddress == null) {
                            _childNodeshtml += '<li><a class="menuItem" data-id="' + row.ModuleId + '" href="#"><i class="fa ' + row.Icon + '"></i><span>' + row.FullName + '</span></a></li>';
                        } else {
                            var url = row.UrlAddress.indexOf("?") >= 0 ? row.UrlAddress + "&mId=" + row.ModuleId : row.UrlAddress + "?mId=" + row.ModuleId;
                            if (row.UrlAddress.indexOf("http") >= 0) {
                                _childNodeshtml += '<li><a class="menuItem" data-id="' + row.ModuleId + '" href="' + url + '"><i class="fa ' + row.Icon + '"></i><span>' + row.FullName + '</span></a></li>';
                            } else {
                                _childNodeshtml += '<li><a class="menuItem" data-id="' + row.ModuleId + '" href="' + contentPath + url + '"><i class="fa ' + row.Icon + '"></i><span>' + row.FullName + '</span></a></li>';
                            }
                        }
                    }
                });
            ...
            $(".sidebar-menu-left").height(($(window).height() - 130));
            $ul.find("li:first").trigger("click");
            //开始菜单
            $(".start_menu").click(function () {
                if (!$(".sidebar-menu").is(":visible")) {
                    $(".sidebar-menu").find(".sidebar-menu-right").css({ "overflow": "auto", height: ($(window).height() - 120) + "px" });
                    $(".sidebar-menu").show();
