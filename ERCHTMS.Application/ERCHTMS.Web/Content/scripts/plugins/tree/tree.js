...
(function ($) {
    ...
    $.fn.treeview = function (settings) {
        ...
        function buildnode(nd, ht, deep, path, isend) {
            ...
            if (nd.IsTrain != undefined) {
                html += " data-istrain='" + nd.IsTrain + "'";
                //ht.push("<span data-value='" + nd.id + "' class='bbit-tree-node-text' unselectable='on' data-code='" + nd.Code + "'>", nd.text, "</span>");
            }
            if (nd.IsOrg != undefined) {
                html += " data-isorg='" + nd.IsOrg + "'";
                //ht.push("<span data-value='" + nd.id + "' class='bbit-tree-node-text' unselectable='on' data-code='" + nd.Code + "'>", nd.text, "</span>");
            }
            ...
            if (nd.IsDept != undefined) {
                html += " data-isdept='" + nd.IsDept + "'";
                //ht.push("<span data-value='" + nd.id + "' class='bbit-tree-node-text' unselectable='on' data-code='" + nd.Code + "'>", nd.text, "</span>");
            }
            if (nd.DeptType != undefined) {
                html += " data-DeptType='" + nd.DeptType + "'";
                //ht.push("<span data-value='" + nd.id + "' class='bbit-tree-node-text' unselectable='on' data-code='" + nd.Code + "'>", nd.text, "</span>");
            }
            if (nd.Nature != undefined) {
                html += " data-DeptNature='" + nd.Nature + "'";
                //ht.push("<span data-value='" + nd.id + "' class='bbit-tree-node-text' unselectable='on' data-code='" + nd.Code + "'>", nd.text, "</span>");
            }
            //else {
            //    ht.push("<span data-value='" + nd.id + "' class='bbit-tree-node-text' unselectable='on' >", nd.text, "</span>");
            ...
            if (dfop.isTool)
            {
                var code = nd.Code == undefined ? "" : nd.Code;
                ...
                for (var ii in dfop.nodeTools) {
                    ...
                    {
                        var span = "<span  data-value='" + nd.id + "' title='" + toolItem.title + "' data-index='"+ii+"'";
                        if (toolItem.className!=undefined) {
                            span+= " class='" + toolItem.className + "'";
                        }  
                        if (toolItem.callback!=undefined) {
                            span += "><a style=\"cursor:pointer;\" onclick=\"" + toolItem.callback + "('" + nd.id + "','" + code + "','" + nd.text + "')\">" + toolItem.text + "</a></span>";
                        }
                        else {
            ...
            if (et.tagName == "IMG") {
                ...
                if ($(et).hasClass("bbit-tree-elbow-plus") || $(et).hasClass("bbit-tree-elbow-end-plus")) {
                    if ($(this).find('i').hasClass('fa-folder')) {
                        $(this).find('i').eq(0).swapClass('fa-folder', 'fa-folder-open');
                    }
                    ...
                    if ($(this).find('i').hasClass('fa-folder-open')) {
                        ...
                        $(this).find('i').eq(0).swapClass('fa-folder-open', 'fa-folder');
                    }
        ...
        function bindevent() {
            $(this).hover(function () {
                $(".bbit-tree-node-tool>span").hide();
                $(this).find(".bbit-tree-node-tool>span").show();
                $(this).addClass("bbit-tree-node-over");
            }, function () {
                $(this).find(".bbit-tree-node-tool>span").hide();
            ...
            if (dfop.isTool) {
                ...
                //        $(".bbit-tree-node-tool span").eq(ii).click(function () {
                ...
                //$(".bbit-tree-node-tool span").click(function () {
                //    var id = $(this).attr("data-value");
                //    var idx = $(this).data("index");
                //    var toolItem = dfop.nodeTools[idx];
                //        if (toolItem.callback != undefined) {
                //             toolItem.callback(id);
                //        }
        ...
        me[0].t = {
            ...
            getSelectedImgs: function () {
                var s = [];
                getck(treenodes, s, function (item) { return item.img; });
                return s;
            },
            getSelectedManagerNames: function () {
                var s = [];
                getck(treenodes, s, function (item) { return item.managerName; });
                return s;
            },
            getSelectedManagerAccounts: function () {
                var s = [];
                getck(treenodes, s, function (item) { return item.managerAccount; });
                return s;
            },
            getSelectedManagers: function () {
                var s = [];
                getck(treenodes, s, function (item) { return item.manager; });
                return s;
            },
    ...
    $.fn.getCheckedImgs = function () {
        if (this[0].t) {
            return this[0].t.getSelectedImgs();
        }
        return null;
    };
    $.fn.getSelectedManagers = function () {
        if (this[0].t) {
            return this[0].t.getSelectedManagers();
        }
        return null;
    };
    $.fn.getSelectedManagerNames = function () {
        if (this[0].t) {
            return this[0].t.getSelectedManagerNames();
        }
        return null;
    };
    $.fn.getSelectedManagerAccounts = function () {
        if (this[0].t) {
            return this[0].t.getSelectedManagerAccounts();
        }
        return null;
    };
    ...
    $.fn.setTreeWidth = function (width) {
        var me = $(this);
        me.width(width);
        //me.parents('.slimScrollDiv').height(height);
    }
