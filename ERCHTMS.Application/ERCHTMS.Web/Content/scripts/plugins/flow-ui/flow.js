    ...
    if (this.$editable) {
        this.$workArea.on("click", { inthis: this }, function (e) {
            ...
            $.each(_nodeData, function (i) {
                if ((_nodeData[i].type == type && type == 'startround') || (_nodeData[i].type == type && type == 'endround')) {
                    alert(name + '节点不能重复');
                    executeadd = false;
                    return false;
                }
...
GooFlow.prototype = {
    ...
    addNode: function (id, json) {
        ...
        var detailtext = "";
        if (json.isclick) {
            clicktext = " onclick=showcurnode('" + id + "','" + json.name + "')";  //业务前台必须配置对应的方法
            detailtext = "<br/><br/>点击查看详情";
        }
        this.$nodeDom[id] = $("<div class='GooFlow_item " + mark + "' id='" + id + "' style='top:" + json.top + "px;left:" + json.left + "px' " + clicktext + "><table cellspacing='1' style='width:" + (json.width) + "px;height:" + (json.height) + "px;'><tr><td class='ico'><b class='ico_" + json.type + "'></b></td><td>" + json.name + detailtext + "</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'></div></div></div>");
