...
$.fn.flowdesign = function (options) {
    ...
    var defaults = {
        ...
        isprocessing: false,
        isshowstatus: "1,2,3,4,5",  //显示状态 1,2,3,4,5
        isshowsetinfo: true,
        nodeData: null,
        activityId: "",
        preview: 0
    ...
    if (options.isprocessing)//如果是显示进程状态
    {
        ...
        if (options.isshowstatus.indexOf('3') >= 0) {
        ...
        if (options.isshowstatus.indexOf('4') >= 0) {
            tipHtml += '<div style="display: inline-block;margin-left: 10px;"><i style="padding-right:5px;color:#f0ad4e;" class="fa fa-flag"></i><span>退回</span></div>';
        }
        if (options.isshowstatus.indexOf('5') >= 0) {
        ...
        $.each(options.nodeData, function (i, item) {
            ...
            $frmpreview.find("#" + item.id).css("background-color", "#999");
            if (item.type == "startround") {
                $frmpreview.find("#" + item.id).css("background-color", "#5cb85c");
            }
            else {
                //其他
                if (item.id == options.activityId) {
                    $frmpreview.find("#" + item.id).css("background-color", "#5bc0de");//正在处理
                }
                if (item.setInfo != undefined && item.setInfo.Taged != undefined && item.setInfo != null) {
                    if (item.setInfo.Taged == -1) {
                        $frmpreview.find("#" + item.id).css("background-color", "#d9534f");//不通过
                    }
                    else if (item.setInfo.Taged == 1) {
                        $frmpreview.find("#" + item.id).css("background-color", "#5cb85c");//通过
                    }
                    else if (item.setInfo.Taged == 0)
                    {
                        $frmpreview.find("#" + item.id).css("background-color", "#5bc0de");//正在处理
                    }
                    else {
                        $frmpreview.find("#" + item.id).css("background-color", "#f0ad4e");//驳回
                    }
                }
            }
            //其他
            if (options.isshowsetinfo) {
                if (item.setInfo != undefined && item.setInfo.Taged != undefined && item.setInfo != null) {
            ...
            if (!(item.url == "null" || item.url == "" || item.url == undefined)) {
                $frmpreview.find('#' + item.id).attr('data-url', item.url);
            }
        });
        $('[data-toggle="tooltip"]').tooltip({ "html": true });
    }
    if (options.preview == 1 && options.isshowsetinfo) {
        preview();
    }
    //单独管理
    ...
    function preview() {
        ...
        $.each(options.flowcontent.nodes, function (i, item) {
            if (item.setInfo != undefined && item.setInfo != null) {
                var _popoverhtml = "";
    ...
    function preview2() {
        $.each(options.flowcontent.nodes, function (i, item) {
            if (item.setInfo != undefined && item.setInfo != null) {
                var _popoverhtml = "";;
                _popoverhtml += '<div class="flow-portal-panel-title"><i class="fa fa-navicon"></i>&nbsp;&nbsp;处理信息</div>';
                _popoverhtml += '<ul style="list-style:none;">';
                if (item.setInfo.NodeDesignateData != undefined) {
                    var _rowstr = "";
                    for (var index = 0; index < item.setInfo.NodeDesignateData.length; index++) {
                        var model = item.setInfo.NodeDesignateData[index];
                        _popoverhtml += '<li>';
                        _popoverhtml += '<ul>';
                        _rowstr = ' <span class="label label-info">' + model.createdate + '</span>';
                        _popoverhtml += '<li>处理时间:' + _rowstr + "</li>";
                        _rowstr = ' <span class="label label-info">' + model.creatdept + '</span>';
                        _popoverhtml += '<li>处理单位:' + _rowstr + "</li>";
                        _rowstr = ' <span class="label label-info">' + model.createuser + '</span>';
                        _popoverhtml += '<li>处理人:' + _rowstr + "</li>";
                        _rowstr = ' <span class="label label-info">' + model.status + '</span>';
                        _popoverhtml += '<li>处理状态:' + _rowstr + "</li>";
                        if (!model.prevnode) { model.prevnode = "无" }
                        _rowstr = ' <span class="label label-info">' + model.prevnode + '</span>';
                        _popoverhtml += '<li>上一流程:' + _rowstr + "</li>";
                        _popoverhtml += '</ul>';
                        _popoverhtml += '</li>';
                        _popoverhtml += '<div class="flow-portal-panel-title"></div>';
                    }
                }
                _popoverhtml += '</ul>';
                $frmpreview.find('#' + item.id).attr('title', item.name);
                ...
                $frmpreview.find('#' + item.id).attr('data-toggle', 'popover');
                $frmpreview.find('#' + item.id).attr('data-placement', 'bottom');
                $frmpreview.find('#' + item.id).attr('data-content', "该节点未执行提交任务");
            }
        });
        $frmpreview.find('.GooFlow_item').popover({ html: true });
    }
