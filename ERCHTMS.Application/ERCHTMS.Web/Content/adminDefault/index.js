...
$.fn.tab = function (options) {
    ...
    function addTab(item) {
        ...
        if (tab.find("#" + item.id).length == 0) {
            ...
            if (item.url) {
                var $iframe = $("<iframe class=\"LRADMS_iframe\" id=\"iframe" + item.id + "\" name=\"iframe" + item.id + "\"  height=\"100%\" width=\"100%\" src=\"" + item.url + "\" frameBorder=\"0\"></iframe>")
...
loadnav = function () {
    ...
    tablist = $("#tab_list_add").tab({
        items: [
            { id: 'desktop', title: '首页', closed: false, icon: 'fa fa fa-desktop', url: contentPath + '/Home/Desktop' },
        ],
        tabScroll: false,
    ...
    $.each(authorizeMenuData, function (i) {
        ...
        if (row.ParentId == '0') {
            index++;
            _html += '<li class="item" data-index="'+index+'">';
    ...
    $('#nav li a').click(function () {
        var id = $(this).attr('id');
        var data = navJson[id];
        if (data.UrlAddress != null) {
            if (data.UrlAddress.indexOf("http") >= 0) {
    ...
    $("#nav li.item").hover(function (e) {
        ...
        if (length > 0) {
            ...
            var sub_top = evt.clientY + $sub.height();
            var body_height = $(window).height();
            if (sub_top > body_height) {
                $sub.css('bottom', '0px');
            } else {
                var top = evt.clientY > 30 ? evt.clientY - 30 : evt.clientY;
                $sub.css("top", top);
            }
    ...
    $("#nav li.sub").hover(function (e) {
        ...
        if (top + height > wheight) {
            $ul.css('top', parseInt('-' + (height - 11))).css('left', '155px')
        } else {
            $ul.css('top', '0px').css('left', '155px');
        }
    }, function (e) {
       //$(this).find('ul').hide();
