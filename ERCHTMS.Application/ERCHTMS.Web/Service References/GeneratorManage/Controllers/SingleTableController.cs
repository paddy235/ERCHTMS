...
namespace ERCHTMS.Web.Areas.GeneratorManage.Controllers
{
    /// <summary>
    /// 描 述：生成器单表
    /// </summary>
    public class SingleTableController : MvcControllerBase
    {
        ...
        [HttpGet]
        [AjaxOnly]
        public ActionResult GetAreaDirs()
        {
            string areaPath = Server.MapPath("~/Areas");
            string[] dirs = Directory.GetDirectories(areaPath);
            List<object> list = new List<object>();
            foreach (string dir in dirs)
            {
                list.Add(new { name = dir.Substring(dir.LastIndexOf("\\") + 1) });
            }
