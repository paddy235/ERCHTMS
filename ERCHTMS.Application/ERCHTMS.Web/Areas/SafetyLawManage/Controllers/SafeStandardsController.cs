...
namespace ERCHTMS.Web.Areas.SafetyLawManage.Controllers
{
    ...
    /// 描 述：安全操作规程
    /// </summary>
    public class SafeStandardsController : MvcControllerBase
    {
        ...
        {
            pagination.p_kid = "Id";
            pagination.p_fields = " a.CreateDate,FileName,IssueDept,FileCode,ValidVersions,CarryDate,FilesId,releasedate,revisedate,lawtypename,a.createuserid,a.createuserdeptcode,a.createuserorgcode ";
            pagination.p_tablename = " bis_safeStandards a ";
            pagination.conditionJson = "1=1";
            
            var data = safestandardsbll.GetPageDataTable(pagination, queryJson);
        ...
        {
            var data = safestandardsbll.GetList(queryJson);
        ...
        {
            var data = safestandardsbll.GetEntity(keyValue);
            return ToJsonResult(data);
        }
        /// <summary>
        /// 获取实体 
        ...
        {
            safestandardsbll.RemoveForm(keyValue);
            return Success("删除成功。");
        }
        /// <summary>
        /// 保存表单（新增、修改）
        ...
        [ValidateAntiForgeryToken]
        [AjaxOnly]
        public ActionResult SaveForm(string keyValue, SafeStandardsEntity entity)
        {
            safestandardsbll.SaveForm(keyValue, entity);
            return Success("操作成功。");
        }
        ...
        public string GetDepartmentCode(SafeInstitutionTreeEntity Entity)
        {
            ...
            var maxObj = departmentBLL.GetDataTable(string.Format("select max(TreeCode) as TreeCode  from bis_safeinstitutiontree t where  parentid='{0}' and CreateUserOrgCode='{1}' and datatype='{2}' ", Entity.ParentId, user.OrganizeCode, Entity.DataType));
