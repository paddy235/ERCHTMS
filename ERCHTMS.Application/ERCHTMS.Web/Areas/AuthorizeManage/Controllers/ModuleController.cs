...
namespace ERCHTMS.Web.Areas.AuthorizeManage.Controllers
{
    ...
    public class ModuleController : MvcControllerBase
    {
        ...
        {
            ViewBag.ModuleId = string.IsNullOrEmpty(Request["keyValue"]) ? Guid.NewGuid().ToString() : Request["keyValue"];
            return View();
        }
        /// <summary>
        /// 功能图标
        ...
        public ActionResult GetTreeJson(string keyword)
        {
            ...
            {
                data = data.TreeWhere(t => t.FullName.Contains(keyword), "ModuleId").OrderBy(t => t.SortCode).ToList();
            }
            ...
            foreach (ModuleEntity item in data)
            {
                ...
                tree.img = item.Icon;
                tree.parentId = string.IsNullOrEmpty(keyword) && item.ParentId == "0" ? "0" : item.ParentId;
                treeList.Add(tree);
            }
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult GetModuleIdByCode(string enCode)
        {
            string moduleId = moduleBLL.GetModuleIdByCode(enCode);
