...
namespace ERCHTMS.Web.Areas.OutsourcingProject.Controllers
{
    ...
    /// 描 述：合同
    /// </summary>
    public class CompactController : MvcControllerBase
    {
        ...
        public ActionResult GetListJson(Pagination pagination, string queryJson)
        {
            ...
            pagination.p_fields = @" e.fullname,r.ENGINEERNAME,to_char(t.compacttakeeffectdate,'yyyy-MM-dd') as compacttakeeffectdate,
                                    to_char(t.compacteffectivedate,'yyyy-MM-dd') as compacteffectivedate,
                                    r.outprojectid,e.senddeptid,t.modifyuserid,t.createuserid,r.engineerletdeptid";
            pagination.p_tablename = @"epg_compact t left join EPG_OutSouringEngineer r 
on t.projectid=r.id left join base_department e on r.outprojectid=e.departmentid ";
            ...
            if (!queryParam["sTime"].IsEmpty() || !queryParam["eTime"].IsEmpty())
            {
                ...
                pagination.conditionJson += string.Format(" and to_date(to_char(compacttakeeffectdate,'yyyy-MM-dd'),'yyyy-MM-dd') between to_date('{0}','yyyy-MM-dd') and  to_date('{1}','yyyy-MM-dd')", startTime, endTime);
            }
            ////查询条件
            ...
            var data = compactbll.GetList(pagination, queryJson);
            var jsonData = new
            {
                rows = data,
        ...
        {
            var data = compactbll.GetEntity(keyValue);
        ...
        /// <returns>返回对象Json</returns>
        [HttpGet]
        public ActionResult GetCompactProtocol(string keyValue)
        {
            var data = compactbll.GetCompactProtocol(keyValue);
            return ToJsonResult(data);
        }
        #endregion
        ...
        {
            compactbll.RemoveForm(keyValue);
            return Success("删除成功。");
        }
        /// <summary>
        /// 保存表单（新增、修改）
        ...
        [ValidateAntiForgeryToken]
        [AjaxOnly]
        public ActionResult SaveForm(string keyValue, CompactEntity entity)
        {
            compactbll.SaveForm(keyValue, entity);
            return Success("操作成功。");
        }
        #endregion
    }
