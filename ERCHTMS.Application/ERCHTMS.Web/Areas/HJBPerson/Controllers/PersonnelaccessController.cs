...
namespace ERCHTMS.Web.Areas.HJBPerson.Controllers
{
    ...
    /// 黄金埠人员进出门禁数据
    /// </summary>
    public class PersonnelaccessController : MvcControllerBase
    {
        ...
        /// <param name="queryJson"></param>
        /// <returns></returns>
        public ActionResult GetData(Pagination pagination,string queryJson)
        {
            var watch = CommonHelper.TimerStart();
            pagination.p_kid = "userid";
            pagination.p_fields = @"devicename,case when inout = 0 then '进门' else '出门' end as inout,CREATEDATE";
            pagination.p_tablename = "(select DISTINCT userid,inout,devicename,CREATEDATE from bis_hikinoutlog)";
            pagination.conditionJson = "1=1";
            var data = hikinoutlogbll.GetTableByUserID(pagination, queryJson);
            var JsonData = new
            {
                rows = data,
                total = pagination.total,
