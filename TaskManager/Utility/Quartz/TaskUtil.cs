...
namespace Utility.Quartz
{
    ...
    {
        private static string InsertSQL = @"INSERT INTO p_Task(TaskID,TaskName,CronExpressionString,Assembly,Class,Status,CronRemark,Remark,LastRunTime,TaskParam)
                            VALUES(@TaskID,@TaskName,@CronExpressionString,@Assembly,@Class,@Status,@CronRemark,@Remark,@LastRunTime,@TaskParam)";
        ...
        private static string UpdateSQL = @"UPDATE p_Task SET TaskName=@TaskName,CronExpressionString=@CronExpressionString,Assembly=@Assembly,
                                Class=@Class,CronRemark=@CronRemark,Remark=@Remark,LastRunTime=@LastRunTime,TaskParam=@TaskParam WHERE TaskID=@TaskID";
        /// <summary>
        /// 获取指定id任务数据
        /// </summary>
        ...
        private static ApiResult<DateTime> GetTaskeLastRunTime(string CronExpressionString)
        {
            ...
            catch (Exception ex)
            {
                ...
                LogHelper.Error("任务Cron表达式设置错误", ex);
            }
            return result;
        }
    }
