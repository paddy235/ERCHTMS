    ...
    public class QuartzHelper
    {
            ...
            try
            {
                lock (obj)
                {
                    if (scheduler == null)
                    {
                        ...
                        scheduler.Clear();
                        //LogHelper.Debug("任务调度初始化成功！");
                    }
                }
            }
            catch (Exception ex)
            {
                LogHelper.Error("任务调度初始化失败！", ex);
            }
        }
                ...
                if (!scheduler.IsStarted)
                {
                    ...
                    if (listTask != null && listTask.Count > 0)
                    {
                        foreach (TaskUtil taskUtil in listTask)
                        {
                            ...
                            {
                                LogHelper.Error(string.Format("任务“{0}”启动失败！", taskUtil.TaskName), e);
                            }
                        }
                    }
                    //LogHelper.Debug("任务调度启动成功！");
                }
            }
            catch (Exception ex)
            {
                LogHelper.Error("任务调度启动失败！", ex);
            }
        }
        ...
        public static void DeleteJob(string JobKey)
        {
            ...
            if (scheduler.CheckExists(jk))
            {
                ...
                LogHelper.Debug(string.Format("任务“{0}”已经删除", JobKey));
            }
        }
        ...
        public static void ScheduleJob(TaskUtil taskUtil, bool isDeleteOldTask = false)
        {
            ...
            if (ValidExpression(taskUtil.CronExpressionString))
            {
                ...
                //else
                //{
                //    LogHelper.Debug(string.Format("任务“{0}”启动成功,未来5次运行时间如下:", taskUtil.TaskName));
                //    List<DateTime> list = GetTaskeFireTime(taskUtil.CronExpressionString, 5);
                //    foreach (var time in list)
                //    {
                //        LogHelper.Debug(time.ToString());
                //    }
                //}
            }
            else
        ...
        public static void PauseJob(string JobKey)
        {
            ...
            if (scheduler.CheckExists(jk))
            {
                ...
                LogHelper.Debug(string.Format("任务“{0}”已经暂停", JobKey));
            }
        }
        ...
        public static void ResumeJob(string JobKey)
        {
            ...
            if (scheduler.CheckExists(jk))
            {
                ...
                LogHelper.Debug(string.Format("任务“{0}”恢复运行", JobKey));
            }
        }
        ...
        public static void RunOnceTask(string JobKey)
        {
            ...
            if (scheduler.CheckExists(jk))
            {
                ...
                var method = type.GetMethod("ExecuteJob");
                object taskParam = null;
                if (method==null)
                {
                ...
                {
                    if (jobDetail.JobDataMap.Get("TaskParam") == null)
                    {
                        taskParam = TaskHelper.GetById(JobKey).TaskParam;
                    }
                    else
                    {
                        taskParam = jobDetail.JobDataMap.Get("TaskParam").ToString();
                    }
                }
                method.Invoke(instance, new object[] {taskParam });
                LogHelper.Debug(string.Format("任务“{0}”立即运行", taskName));
            }
        }
        /// 获取类的属性、方法  
        /// </summary>  
        ...
        public static void StopSchedule()
        {
            try
            {
                ...
                if (!scheduler.IsShutdown)
                {
                    ...
                    LogHelper.Debug("任务调度停止！");
                }
            }
            catch (Exception ex)
            {
                LogHelper.Error("任务调度停止失败！", ex);
            }
        }
